...
lines = open('scripts/senior-rebuild/required2/_raw/full.txt', encoding='utf-8').read().split('\n')
# 词汇表正文:找"词汇表"标记后,到"后记"/不规则动词表前
start = next(i for i,l in enumerate(lines) if l.strip()=='词汇表')
# 不规则动词表大致从 "Irregular" 或 后记;用页123(后记)截断
end = next(i for i,l in enumerate(lines) if '后' in l and '记' in l and i>start)
...
for u in byunit:
    json.dump(byunit[u], open(f'scripts/senior-rebuild/required2/_raw/wl_u{u}.json','w',encoding='utf-8'), ensure_ascii=False, indent=1)
