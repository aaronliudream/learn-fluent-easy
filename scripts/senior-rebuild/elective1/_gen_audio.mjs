// 预生成选择性必修一全单元听力音频(edge TTS, nova/0.85, 与关7同口径),产出 audio_url UPDATE SQL。
// 内容寻址:synth(cleanForTTS(transcript)) 得到的 CF URL = 播放路径请求的同一 URL。
// transcript 构造必须与 _gen_load.mjs 完全一致(speaker: text 用 \n 连接),否则 hash 不符。
// 用法: node scripts/senior-rebuild/elective1/_gen_audio.mjs
...
for (const u of Object.keys(META)) {
  const m = META[u];
  const lf = JSON.parse(readFileSync(`scripts/senior-rebuild/elective1/${u}/${VOL}-${u}-listening.json`, 'utf8'));
...
let sql = `-- 选择性必修一 听力 audio_url 回填(${ok}/${total} 合成成功)。content-addressed CF URL。Aaron 先跑各单元 load,再跑本文件。幂等。\nBEGIN;\n`;
...
writeFileSync('SQLAA/elective1-listening-audio-url.sql', sql);
console.log(`\n合成 ${ok}/${total} -> SQLAA/elective1-listening-audio-url.sql`);
