    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center">
        <div>
          ...
          <p className="mb-3 text-slate-700"><T>登录后即可看到同学、参与互动～</T></p>
          <Link to="/auth" className="inline-block rounded-xl bg-amber-500 px-6 py-2 font-medium text-white"><T>去登录</T></Link>
        </div>
      </div>);
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-rose-50 to-sky-50 pb-24">
      ...
      <div className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
          <BackLink to="/" className="inline-flex items-center gap-1 text-slate-600 hover:text-slate-900"><ArrowLeft className="h-4 w-4" /><T>返回</T></BackLink>
          <h1 className="text-base font-bold text-slate-800"><T>🌟 同学社区</T></h1>
        ...
        <div className="mx-auto flex max-w-3xl gap-1 overflow-x-auto px-2 pb-2 text-xs">
          ...
          ["wave", "打招呼", Hand],
          ["coop", "合作答题", Users]] as
          [Tab, string, any][]).map(([k, label, Icon]) =>
      ...
      <div className="mx-auto max-w-3xl px-4 pt-4">
        {loading && <div className="flex items-center justify-center py-10 text-slate-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" /><T>加载中…</T></div>}

        {!loading && tab === "feed" &&
        <div className="space-y-4">
            <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-sm font-bold text-slate-700"><T>🟢 现在</T> {online} <T>位同学正在学习</T></div>
              </div>
              ...
              <span key={o.user_id} className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-xs text-emerald-700 ring-1 ring-emerald-100">
                ...
                {onlineList.length === 0 && <span className="text-xs text-slate-400"><T>暂无其他同学在线，做第一个吧～</T></span>}
              </div>
            </div>
            <div className="space-y-2">
              {feed.length === 0 && <div className="rounded-2xl bg-white p-6 text-center text-sm text-slate-500 ring-1 ring-slate-200"><T>还没有动态，先去答题或喂宠物吧！</T></div>}
              {feed.map((f) =>
        ...
        <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
              <div className="mb-3 flex items-center gap-2 font-bold text-slate-800"><Coins className="h-4 w-4 text-amber-500" /><T>今日星币榜</T></div>
              ...
              <li key={r.user_id} className="flex items-center gap-2 text-sm">
                ...
                {coinRank.length === 0 && <li className="text-xs text-slate-400"><T>暂无数据</T></li>}
              </ol>
            </div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
              <div className="mb-3 flex items-center gap-2 font-bold text-slate-800"><Trophy className="h-4 w-4 text-rose-500" /><T>本周宠物等级榜</T></div>
              <ol className="space-y-2">
                {petRank.map((r, i) =>
              <li key={r.user_id} className="flex items-center gap-2 text-sm">
                    <span className={cn("inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold", i === 0 ? "bg-amber-400 text-white" : i === 1 ? "bg-slate-300 text-white" : i === 2 ? "bg-orange-300 text-white" : "bg-slate-100 text-slate-600")}>{i + 1}</span>
                    <span className="text-lg">{r.pet_emoji}</span>
                    <span className="flex-1 truncate">{r.username} <T>的</T> {r.pet_name}</span>
                    <span className="font-bold text-rose-600">Lv.{r.level}</span>
                  </li>
              )}
                {petRank.length === 0 && <li className="text-xs text-slate-400"><T>暂无数据</T></li>}
              </ol>
            </div>
          </div>
        }
              ...
              return (
                <div key={l.id} className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500">{l.username} <T>· 单价</T> {l.price_per_unit}⭐</div>
                    </div>
                    {isMine ?
                  <button onClick={async () => {await supabase.rpc("cancel_listing", { _listing_id: l.id });loadTab("market");}} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600"><T>下架</T></button> :

                  <button onClick={() => buy(l)} className="rounded-full bg-amber-500 px-3 py-1 text-xs font-bold text-white shadow">{total}<T>⭐ 买下</T></button>
                  }
                  </div>);

            })}
              {listings.length === 0 && <div className="col-span-full rounded-2xl bg-white p-6 text-center text-sm text-slate-500 ring-1 ring-slate-200"><T>市集暂时空空，挂一份你的多余粮食吧～</T></div>}
            </div>
          </div>
        }
        ...
        <div className="space-y-3">
            <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
              <div className="text-sm text-slate-700"><T>👋 今日还可给</T> <b className="text-emerald-600">{waveQuota}</b> <T>位同学打招呼（送对方 1 星币加油）</T></div>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              {onlineList.map((o) =>
            <div key={o.user_id} className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
                  ...
                  <button disabled={waveQuota <= 0} onClick={() => waveTo(o.user_id)} className={cn("rounded-full px-3 py-1 text-xs font-bold text-white shadow", waveQuota > 0 ? "bg-emerald-500" : "bg-slate-300")}><T>✨ 加油</T></button>
                </div>
            )}
              {onlineList.length === 0 && <div className="col-span-full rounded-2xl bg-white p-6 text-center text-sm text-slate-500 ring-1 ring-slate-200"><T>还没有同学在线～</T></div>}
            </div>
          </div>
        }
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-500 p-6 text-center text-white shadow-lg">
                ...
                <h3 className="mb-1 text-lg font-bold"><T>🤝 合作答题</T></h3>
                <p className="mb-4 text-sm opacity-90"><T>和 1-2 位同学组队，30 分钟内共同答对 20 题，每人 +50 星币 + 1 份稀有粮食！</T></p>
                <button onClick={joinCoop} className="rounded-full bg-white px-6 py-2 text-sm font-bold text-fuchsia-700 shadow"><T>立即匹配</T></button>
              </div> :

          <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
                <div className="mb-2 text-sm text-slate-600"><T>本队进度</T></div>
                <div className="mb-1 flex items-end justify-between">
                  <div className="text-2xl font-bold text-slate-800">{coopState.current_correct} / {coopState.goal_correct}</div>
                  <div className="text-xs text-slate-500"><T>状态：</T>{coopState.status === "open" ? "等待队友…" : "进行中"}</div>
                </div>
                ...
                <p className="mt-3 text-xs text-slate-500"><T>💡 现在去任意答题游戏，每答对一题都会自动为本队 +1。</T></p>
                <div className="mt-3 flex gap-2">
                  <Link to="/primary" className="rounded-full bg-amber-500 px-4 py-1.5 text-xs font-bold text-white"><T>小学游戏</T></Link>
                  <Link to="/junior" className="rounded-full bg-sky-500 px-4 py-1.5 text-xs font-bold text-white"><T>初中练习</T></Link>
                </div>
              </div>
          }
          </div>
...
function ListFoodForm({ foods, inv, onDone, showToast }: {foods: Food[];inv: {food_id: string;qty: number;}[];onDone: () => void;showToast: (m: string) => void;}) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
      <div className="mb-2 text-sm font-bold text-slate-800"><T>📦 上架我的粮食</T></div>
      {owned.length === 0 ?
      <p className="text-xs text-slate-500"><T>你还没有可上架的粮食 —— 先去宠物商店买一些，或答题获得！</T></p> :
      ...
      <div className="flex flex-wrap items-center gap-2">
          <select value={foodId} onChange={(e) => setFoodId(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-2 py-1 text-sm">
            <option value=""><T>选粮食…</T></option>
            {owned.map((i) => {const f = foods.find((x) => x.id === i.food_id);return <option key={i.food_id} value={i.food_id}>{f?.emoji} {f?.name_cn} <T>(持有</T> {i.qty})</option>;})}
          </select>
          <label className="text-xs text-slate-500"><T>数量</T><input type="number" min={1} max={99} value={qty} onChange={(e) => setQty(+e.target.value)} className="ml-1 w-16 rounded-xl border border-slate-200 px-2 py-1" /></label>
          <label className="text-xs text-slate-500"><T>单价</T><input type="number" min={1} max={999} value={price} onChange={(e) => setPrice(+e.target.value)} className="ml-1 w-20 rounded-xl border border-slate-200 px-2 py-1" />⭐</label>
          <button onClick={submit} className="ml-auto rounded-full bg-slate-900 px-4 py-1.5 text-xs font-bold text-white"><T>挂单</T></button>
        </div>
      }
    </div>);
...
function GiftPanel({ quota, foods, inv, online, onSend }: {quota: number;foods: Food[];inv: {food_id: string;qty: number;}[];online: any[];onSend: (toUser: string, foodId: string) => void;}) {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
        <div className="mb-2 text-sm text-slate-700"><T>🎁 今日还可免费送出</T> <b className="text-rose-600">{quota}</b> <T>份粮食</T></div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <div key={o.user_id} className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-slate-200">
            <div className="text-xl">{o.emoji}</div>
            <div className="flex-1 truncate text-sm">{o.username}</div>
            <button disabled={quota <= 0 || !foodId} onClick={() => onSend(o.user_id, foodId)} className={cn("rounded-full px-3 py-1 text-xs font-bold text-white shadow", quota > 0 && foodId ? "bg-rose-500" : "bg-slate-300")}><T>送出</T></button>
          </div>
        )}
        {online.length === 0 && <div className="col-span-full rounded-2xl bg-white p-6 text-center text-sm text-slate-500 ring-1 ring-slate-200"><T>暂无在线同学，过会儿再来～</T></div>}
      </div>
    </div>);
