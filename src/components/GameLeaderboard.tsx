...
export default function GameLeaderboard({
  ...
  gameType: "word_rush" | "word_bento" | "word_quest" | "word_duel";
  title: string;
  accent?: "fuchsia" | "amber" | "emerald" | "indigo";
}) {
    ...
    <div className={cn("rounded-3xl border-2 bg-gradient-to-br p-5 shadow-tile", accentClasses)}>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-extrabold">{title} 排行榜</h3>
        </div>
      ...
      {my && my.total_plays > 0 && (
        <div className="mb-3 rounded-2xl border bg-card/80 p-3">
          <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">我的战绩</div>
          <div className="grid grid-cols-4 gap-2 text-center">
            ...
            <div>
              ...
              <div className="text-[10px] text-muted-foreground">局数</div>
            </div>
            <div>
              <div className="text-lg font-extrabold">
                {my.week_rank > 0 ? `#${my.week_rank}` : "—"}
              </div>
              <div className="text-[10px] text-muted-foreground">周排名</div>
            </div>
          </div>
        </div>
      )}
        ...
        <ol className="space-y-1.5">
          {rows.slice(0, 20).map((r) => (
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">{r.total_plays} 局</span>
