/**
 * 单元详情页 — 游戏化版 (Unit Gamified PR_1 骨架)
 *
 * 路由白名单 unit (GAMIFIED_UNIT_IDS) 走这个组件, 其余继续走老的
 * PrimaryHubUnit.tsx (老文件 0 改动).
 *
 * PR_1 的本组件视觉 + 行为 = 老组件的 1:1 复制 (只为验证路由分叉本身).
 * PR_2 会重写 body: 焦点卡 + 闯关路径 + 已完成折叠 + Boss 终点徽章.
 *
 * 设计 token: 已经在 src/index.css 全局可用 (--fc-*); 不引新 css 文件.
 */
...
export default function PrimaryHubUnitGamified() {
  ...
  return (
    <>
      {/* PR_1 骨架 — 视觉与老组件一致. PR_2 会重写下面整段为游戏化布局. */}
