...
总之,虽然网络购物提供了众多优势,但也存在消费者必须应对的挑战。平衡便利性与潜在缺点是获得满意的网络购物体验的关键。', 1, false),
  ('租房搬家', 'renting a flat', 'Renting a apartment involves several steps. First, you view a apartment to find a suitable one. After that, you sign a lease and pay a deposit. Setting up utilities is next, followed by arranging furniture and moving in. Meeting the neighbors is important for community integration. 
...
SELECT p.id, v.kind, v.text_en, v.text_zh, w.id, v.sort_order
  FROM (VALUES
    ...
    ('租房搬家', 'chunk', 'view a apartment', '看房', NULL, 1),
