...
import {
  ...
  CheckCircle2, Circle, Clock, Flame, TrendingUp, Award, Zap, BookMarked, ChevronRight,
} from "lucide-react";
...
export default function GaokaoReadingKnowledge() {
  ...
  const catStats = useMemo(() => {
    ...
    for (const k of kps) {
      ...
      else if (s === "learning") st.learning++;
    }
    return stats;
  }, [kps, mastery]);
          ...
          <div className="relative shrink-0 mx-auto sm:mx-0">
            ...
            <div className="absolute inset-0 grid place-items-center text-center">
              <div>
                ...
                <div className="text-[10px] text-muted-foreground mt-1">总掌握度</div>
              </div>
            </div>
          </div>
        ...
        {recommendations.length > 0 && (
          <div className="relative mt-5 pt-5 border-t border-border/60">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <h3 className="font-bold text-sm">今日推荐 · 高频必学</h3>
              <span className="text-[10px] text-muted-foreground">基于考试频率与你的进度</span>
            </div>
                ...
                return (
                  <button key={k.id} onClick={() => { setActiveCat(k.category_code); setOpenKP(k); }}
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5">
                      ...
                      {k.exam_frequency && <span className={cn("rounded-full border px-1.5 py-0.5 text-[10px] font-semibold", FREQ_BADGE[k.exam_frequency] ?? "")}>{k.exam_frequency}频</span>}
                    </div>
        ...
        {[
          ...
          return (
            <button key={t.k} onClick={() => setTab(t.k as any)}
              ...
              <div className="flex items-center gap-1.5 text-xs opacity-80"><Icon className="size-3.5" />{t.count} 条</div>
        ...
        <>
          ...
          <div className="mb-4 flex flex-wrap gap-1.5 items-center text-xs">
            ...
            {([
              ...
              { k: "mastered", label: "已掌握", c: "before:bg-emerald-500" },
            ] as const).map((s) => (
