...
export default function GaokaoGrammarPoint() {
  ...
  if (!point) return <p className="p-8"><T>考点不存在。</T><BackLink to="/gaokao/grammar" className="text-primary underline"><T>返回</T></BackLink></p>;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-8 pb-24">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-muted-foreground">
              ...
              <span className="ml-2 text-rose-600 font-bold"><T>⚠ 已到复习时间</T></span>
              }
            </div>
          </div>
      ...
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-4">
            <div className="text-xs font-bold uppercase tracking-wider text-primary mb-1"><T>🎯 快速诊断 ·</T> {diagIdx + 1}/{diagQuestions.length}</div>
            <h3 className="text-lg font-bold"><T>3 道题定位你的起点</T></h3>
            <p className="text-xs text-muted-foreground mt-1"><T>不是考试，目的是帮系统选最适合你的学习路径</T></p>
          </div>
        ...
        <>
          ...
          <div className="mt-4 rounded-xl bg-muted/50 p-4 text-sm">
                  <div className="mb-1 font-bold text-xs"><T>解析</T></div>
      ...
      {stage === "notice" &&
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-3 text-xs font-bold uppercase tracking-wider text-primary"><T>👀 观察对比 · 先看，再总结规律</T></div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border-2 border-rose-300 bg-rose-50 dark:bg-rose-950/20 dark:border-rose-900 p-4">
                <div className="text-xs font-bold text-rose-600 mb-2"><T>❌ 常见错误</T></div>
              ...
              <div className="rounded-xl border-2 border-emerald-300 bg-emerald-50 dark:bg-emerald-950/20 dark:border-emerald-900 p-4">
                <div className="text-xs font-bold text-emerald-600 mb-2"><T>✅ 正确表达</T></div>
                <div className="text-sm leading-relaxed whitespace-pre-wrap">{point.typical_example || "见下一阶段拆解"}</div>
              </div>
            </div> :

        <div className="rounded-xl bg-muted/40 p-4 text-sm text-muted-foreground">
              <T>本考点暂无对比例句，请继续到"拆解"阶段。</T>
            </div>
        }
          <div className="mt-4 rounded-xl bg-primary/5 border border-primary/20 p-3 text-xs">
            💡 <strong><T>停一下：</T></strong><T>你能看出区别在哪里吗？带着这个问题进入下一步会更高效。</T>
          </div>
          <div className="mt-4 flex gap-2">
            <Button variant="outline" onClick={() => setStage("diagnose")}><T>← 上一步</T></Button>
            <Button onClick={() => setStage("explain")} className="flex-1"><T>下一步：看拆解 →</T></Button>
          </div>
        </section>
      }
      ...
      {stage === "explain" &&
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-3 text-xs font-bold uppercase tracking-wider text-primary"><T>💡 拆解 · 公式 + 例句 + 易错点</T></div>
        ...
        <div className="mt-4 rounded-xl border-l-4 border-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/20 p-3">
              <div className="text-xs font-bold text-emerald-700 dark:text-emerald-400 mb-1"><T>📌 典型例句</T></div>
              <p className="text-sm whitespace-pre-wrap">{point.typical_example}</p>
            </div>
        }
          <div className="mt-4 flex gap-2">
            <Button variant="outline" onClick={() => setStage("notice")}><T>← 上一步</T></Button>
      ...
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-3 flex items-center justify-between text-xs">
            <span className="font-bold uppercase tracking-wider text-primary"><T>🏋️ 练习 ·</T> {Math.min(pracIdx + 1, practiceQuestions.length)}/{practiceQuestions.length}</span>
            <span className="text-muted-foreground"><T>本轮 ✓</T>{stats.correct} · ✗{stats.wrong}</span>
          </div>
          {q ?
        <>
          ...
          <div className="mb-3 rounded-xl bg-muted/40 p-3 text-xs">
                  <div className="mb-2 text-muted-foreground"><T>看完题目，先评估你的把握 (元认知训练)：</T></div>
          ...
          <div className="mt-4 rounded-xl border-2 border-amber-400 bg-amber-50 dark:bg-amber-950/20 p-3">
                  <div className="text-xs font-bold mb-2"><T>📝 这道题为什么错？(帮系统给你推对的题)</T></div>
        ...
        <p className="text-sm text-muted-foreground"><T>该考点暂无题目。</T></p>
        }
        </section>
      }
      ...
      {stage === "apply" &&
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-3 text-xs font-bold uppercase tracking-wider text-primary"><T>🎯 真题应用</T></div>
          <div className="rounded-xl bg-muted/30 p-4 text-sm">
            <p className="mb-3"><T>本轮成绩：</T><strong className="text-emerald-600">{stats.correct}</strong> <T>对 /</T> <strong className="text-rose-600">{stats.wrong}</strong> <T>错 (</T>{practiceQuestions.length ? Math.round(stats.correct / practiceQuestions.length * 100) : 0}%)</p>
            <p className="text-muted-foreground text-xs">
              <T>建议接下来：在阅读训练中遇到本考点的句子时，会自动高亮提醒你。也可以做几道更难的混合题。</T>
            </p>
          </div>
          <div className="mt-4 flex gap-2">
            <Button variant="outline" onClick={reset}><RotateCcw className="mr-1 size-4" /> <T>再练一轮</T></Button>
            <Button onClick={() => setStage("reflect")} className="flex-1"><T>下一步：复盘 →</T></Button>
          </div>
        </section>
      }
      ...
      {stage === "reflect" &&
      <section className="rounded-2xl border bg-card p-5">
          <div className="mb-3 text-xs font-bold uppercase tracking-wider text-primary"><T>📋 学完复盘</T></div>
          <div className="text-center py-4">
            ...
            <span className="block mt-1">
                  <T>下次复习：</T>{new Date(mastery.due_at).toLocaleDateString("zh-CN", { month: "short", day: "numeric" })}
                </span>
            }
            </p>
          </div>
        ...
        <div className="rounded-xl bg-primary/5 border border-primary/20 p-3 text-xs">
              ...
              <ul className="mt-1.5 ml-4 list-disc space-y-0.5 text-muted-foreground">
                {(mastery?.correct_count ?? 0) < 12 && <li><T>累计答对</T> {12 - (mastery?.correct_count ?? 0)} <T>题</T></li>}
                {(mastery?.stability ?? 0) < 21 && <li><T>记忆抗遗忘力达 21 天 (当前</T> {Math.round(mastery?.stability ?? 0)} <T>天)</T></li>}
                {!mastery?.reached_master_at && <li><T>间隔 ≥7 天后再次答对 (证明长期记住了)</T></li>}
              </ul>
            </div>
        }
          <div className="mt-4 flex gap-2">
            <Button variant="outline" asChild className="flex-1"><BackLink to="/gaokao/grammar"><T>返回地图</T></BackLink></Button>
            <Button onClick={reset} className="flex-1"><RotateCcw className="mr-1 size-4" /> <T>再练一轮</T></Button>
          </div>
        </section>
      }
    </main>);
