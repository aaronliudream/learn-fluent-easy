...
describe("sentenceRegistry auto-discovery", () => {
  ...
  it("does not register unrelated units", () => {
    expect(getSentenceLesson("g4v2_u3", 3)).toBeNull();
  });

  it("discovers sentence lessons for u1 and u2", () => {
    expect(__getSentenceLessonsForTest()).toHaveLength(2);
  });
});
...
describe("readWriteRegistry auto-discovery", () => {
  ...
  it("does not register unrelated units", () => {
    expect(getReadWriteConfig("g4v2_u3", 6)).toBeNull();
  });
});
