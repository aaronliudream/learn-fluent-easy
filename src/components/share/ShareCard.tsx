import { T } from "@/i18n/T";import { forwardRef, useEffect, useState } from "react";
  ...
  return (
    <div
      ...
      <div style={{ position: "absolute", top: 60, left: 80, right: 80, display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 28, fontWeight: 800, letterSpacing: 1 }}><T>大月亮英语</T></div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 28, fontWeight: 800 }}><T>扫码即学 · Scan to learn</T></div>
          <div style={{ fontSize: 18, opacity: 0.85, marginTop: 6 }}>bigmoonenglish.com</div>
          <div style={{ fontSize: 16, opacity: 0.7, marginTop: 4 }}><T>AI 个性化英语学习 · 初中 · 高考</T></div>
        </div>
      </div>
    </div>);
...
function CardBody({ item, accent }: {item: ShareItem;accent: ReturnType<typeof pickAccent>;}) {
  if (item.type === "word") {
    return (
      <div>
        <div style={{ fontSize: 28, opacity: 0.8 }}><T>📚 今日单词 · Word of the day</T></div>
  ...
  if (item.type === "score") {
    return (
      <div style={{ textAlign: "center", paddingTop: 40 }}>
        ...
        <div style={{ fontSize: 28, opacity: 0.8, marginTop: 6 }}><T>分 / Score</T></div>
        {item.rank && <div style={{ marginTop: 40, display: "inline-block", padding: "16px 40px", borderRadius: 999, background: "rgba(255,255,255,0.18)", fontSize: 32, fontWeight: 700 }}><T>🥇 全站前</T> {item.rank}%</div>}
      </div>);
