  ...
  const send = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (status === 429) {
          setQuotaHit(true);
          setMsgs(m => [...m, { role: "assistant", content: "今天和我聊天的次数用完啦，明天再来吧 🌙" }]);
        } else {
          setMsgs(m => [...m, { role: "assistant", content: "我有点累了，等会儿再聊好吗？" }]);
        }
        return;
      }
      ...
      setMsgs(m => [...m, { role: "assistant", content: "网络好像断了，再试一次吧。" }]);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex h-[420px] flex-col rounded-2xl border-2 border-border bg-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-border bg-gradient-to-r from-violet-500/10 to-fuchsia-500/10 px-3 py-2">
        ...
        {remaining !== null && (
          <span className="text-[10px] text-muted-foreground">今日剩余 {remaining} 轮</span>
        )}
      </div>
        ...
        {busy && (
          <div className="flex justify-start">
            <div className="rounded-2xl bg-muted px-3 py-2 text-xs text-muted-foreground">
              <Loader2 className="inline size-3 animate-spin" /> {petName} 在想…
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 border-t border-border bg-background p-2">
        <input
          ...
          maxLength={300}
          placeholder={quotaHit ? "今日额度用完了" : "说点什么…"}
