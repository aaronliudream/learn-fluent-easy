  ...
  if (loading) {
    return (
      <section className="mb-4 rounded-3xl border-2 border-border bg-card p-4 text-sm text-muted-foreground">
        <Loader2 className="mr-2 inline size-4 animate-spin" /> <T>加载测验设置…</T>
      </section>);
    ...
    <section className="mb-4 rounded-3xl border-2 border-border bg-card p-4 shadow-tile md:p-5">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-extrabold"><T>🧠 测验节奏 & 题型比例</T></div>
            <div className="text-[11px] text-muted-foreground"><T>家长按孩子年龄调整每轮题数和题型分布</T></div>
          </div>
        </div>
      ...
      <div className="mb-4">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="font-bold"><T>每轮题数</T></span>
          <span className="text-muted-foreground"><T>当前</T> <b className="text-pink-600">{size}</b> <T>题/轮</T></span>
        </div>
      ...
      {/* 题型比例 — 预设 */}
      <div className="mb-3">
        <div className="mb-1.5 text-xs font-bold"><T>题型比例（一键预设）</T></div>
      ...
      <div className="rounded-2xl border border-border bg-secondary/30 p-3">
        <div className="mb-2 flex items-center gap-1 text-xs font-bold">
          <Target className="size-3.5 text-violet-500" /> <T>自定义比例（可微调）</T>
        </div>
        ...
        <div className="mt-2 text-[10px] text-muted-foreground">
          <T>每轮</T> {size} <T>题，按上述比例随机抽取并打乱顺序</T>
        </div>
      </div>
    </section>);
...
}: {label: string;color: string;value: number;pct: number;onMinus: () => void;onPlus: () => void;}) {
  return (
    <div>
