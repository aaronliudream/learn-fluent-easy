...
if (!META[U]) throw new Error('未知 unit ' + U);
const DIR = `scripts/senior-rebuild/fltrp-${VOL}/${U}`;
...
let g = JSON.parse(readFileSync(fp('grammar'), 'utf8')); console.log('grammar', rebal(g.questions)); writeFileSync(fp('grammar'), JSON.stringify(g, null, 2) + '\n');
let r = JSON.parse(readFileSync(fp('reading'), 'utf8')); let rq = []; r.passages.forEach(p => p.questions.forEach(q => rq.push(q))); console.log('reading', rebal(rq)); writeFileSync(fp('reading'), JSON.stringify(r, null, 2) + '\n');
let c = JSON.parse(readFileSync(fp('cloze'), 'utf8')); let cq = []; c.passages.forEach(p => p.questions.forEach(q => cq.push(q))); console.log('cloze', rebal(cq)); writeFileSync(fp('cloze'), JSON.stringify(c, null, 2) + '\n');
let s = JSON.parse(readFileSync(fp('listening'), 'utf8')); let sq = []; s.exercises.forEach(e => e.questions.forEach(q => sq.push(q))); console.log('listening', rebal(sq)); writeFileSync(fp('listening'), JSON.stringify(s, null, 2) + '\n');
