-- ============================================================================
-- 图书馆「绘本模式」· 伊索寓言 ch33《朱庇特与乌龟》分页(3 页 / 3 张图)
--
-- ⚠️ 抗切分写法(ch8 起的默认约定):不用任何 do $$ 块、注释里不留分号、
--    断言用 CASE + 强制类型转换。
--
-- 分页口径:本章只有 1 段,按 seq 升序 row_number() 硬切 2/2/1,para_idx 一个字不动。
--    页1(句1-2)= 朱庇特成婚遍请百兽赴宴 · 唯独乌龟没到 朱庇特很意外
--    页2(句3-4)= 下回路上遇见 问他为何缺席 · 乌龟答「哪儿都不如家里好」
--    页3(句5)  = 朱庇特恼了 当即判乌龟从此把家背在身上 再想出门也离不成
--
-- ✅ 3 张图已传桶(2026-07-29,1200 宽 q82),公开地址实测全部 HTTP 200
--    library-illustrations/aesop-easy-readers/ch33/p{1,2,3}.jpg
--
-- 画风:卡通彩色。
--
-- 画面验收要点:
--   ① **时序红线**:龟壳变成小屋只能出现在第 3 页。前两页必须是普通自然龟壳 ——
--      判决是本章的结局,提前画出来笑点就没了。三张图实测符合。
--   ② 第 1 页乌龟**完全不出现**:缺席全靠席末那把空木椅 + 一只没动过的金盘,
--      加上朱庇特侧头望过去、摊开手掌的困惑表情。
--   ③ 姿态弧线:朱庇特(困惑摊手 → 俯身发问 → 皱眉举指宣判、掌心发金光)、
--      乌龟(不在画面 → 昂首得意 指着自己的洞 → 缩颈低头 认命)。
--   ④ 五位赴宴动物种类体型各不相同(棕兔/灰鼠/白鸭/棕刺猬/褐狐),彼此完整分开。
--      朱庇特与乌龟三页全程零接触、中间始终留大片空地。无红色。
--
-- 影响面:只碰 book_key='aesop-easy-readers' 且 chapter_idx=33 的 5 行。幂等:再跑无害。
-- ⚠️ 跑完还需前端把 'aesop-easy-readers#33' 加进 PICTURE_BOOK_CHAPTERS。
-- ============================================================================

begin;

-- 前置计数:本章应为 5 句、1 段、page_index 全空
select count(*) as ch33_rows,
       count(distinct s.para_idx) as ch33_paras,
       count(s.page_index) as ch33_paged_before
from public.library_sentences s
join public.library_books b on b.id = s.book_id
where b.book_key = 'aesop-easy-readers'
  and s.chapter_idx = 33;

-- 🔒 前置硬闸:句数必须恰好 5
select case
         when count(*) = 5
           then 'OK 前置 ch33 恰好 5 句 硬编码切点 1-2/3-4/5 成立'
         else ('前置硬闸失败 ch33 句数=' || count(*) ||
...
select count(distinct s.para_idx) as ch33_paras_after
from public.library_sentences s
join public.library_books b on b.id = s.book_id
where b.book_key = 'aesop-easy-readers'
  and s.chapter_idx = 33;
...
-- ch1-ch32 未被本单影响自检:期望 32 章(ch32 是 2 页 2 图 其余 31 章 3 页 3 图)
select count(*) as chapters_1_to_32_ok
from (
  select s.chapter_idx
  from public.library_sentences s
  join public.library_books b on b.id = s.book_id
  where b.book_key = 'aesop-easy-readers'
    and s.chapter_idx between 1 and 32
    and s.page_index is not null
