    ...
    <main className="mx-auto min-h-screen max-w-3xl px-4 py-4 pb-32">
      ...
      <div className="sticky top-0 z-20 -mx-4 mb-4 border-b border-border bg-background/85 px-4 py-2 backdrop-blur">
        <div className="flex items-center justify-between gap-2">
          <Link to="/gaokao/cloze" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
            <ArrowLeft className="size-3.5" /> 列表
          </Link>
      ...
      <header className="mb-4">
        <div className="mb-1 flex items-center gap-2 text-[11px] text-muted-foreground">
          ...
          <span>·</span><span>{passage.word_count} 词</span>
        </div>
        ...
        <div className="fixed inset-x-0 bottom-20 z-30 mx-auto max-w-3xl px-4">
          <div className="rounded-2xl border border-border bg-card/95 p-3 shadow-xl backdrop-blur">
            <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
              <span>第 {activeBlank} 空 · 选一个最佳答案</span>
      ...
      {!submitted && (
        <div className="mt-4 rounded-2xl border border-border bg-card p-3">
          <div className="mb-2 text-xs text-muted-foreground">快速跳转</div>
      ...
      {submitted && (
        <section className="mt-6 space-y-2">
          <h2 className="mb-2 flex items-center gap-2 text-base font-semibold">
            <Lightbulb className="size-4 text-amber-500" /> 逐题精讲
          </h2>
          {blanks.map((b) => {
                ...
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <span className="text-sm">
                      你选 <b>{ans || "—"}</b> · 正确 <b className="text-emerald-700 dark:text-emerald-400">{b.correct_answer}</b>
                    </span>
                  ...
                  <div className="space-y-3 border-t border-border/50 bg-background/50 px-3 py-3 text-sm">
                    {b.skill_method && (
                      <div className="rounded-lg bg-amber-500/10 px-3 py-2 text-amber-900 dark:text-amber-100">
                        <div className="mb-0.5 text-[11px] font-semibold uppercase tracking-wide opacity-70">解题方法</div>
                        {b.skill_method}
                      </div>
                    )}
                        ...
                        return (
                          <div key={L} className={cn("rounded-lg border px-3 py-2 text-xs",
                            ...
                            <div className="flex items-center gap-1.5 font-semibold">
                              ...
                              <span>{opt}</span>
                              {isCorrect && <span className="ml-auto text-[10px] text-emerald-600">✓ 正确</span>}
                              {isUserPick && !isCorrect && <span className="ml-auto text-[10px] text-rose-600">你的选择</span>}
                            </div>
      ...
      {submitted && (
        <section className="mt-6 space-y-2">
          <button onClick={() => setShowTranslation((v) => !v)} className="flex w-full items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-sm font-semibold hover:bg-muted/50">
            <span className="inline-flex items-center gap-2"><Languages className="size-4" /> 全文翻译</span>
          ...
          <button onClick={() => setShowAnalysis((v) => !v)} className="flex w-full items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-sm font-semibold hover:bg-muted/50">
            <span className="inline-flex items-center gap-2"><Sparkles className="size-4" /> 文章脉络 & 考点综述</span>
          ...
          {showAnalysis && (
            <div className="space-y-3 rounded-xl border border-border bg-card px-5 py-4 text-sm leading-relaxed">
              {passage.article_analysis && <div><div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">文章解析</div><p>{passage.article_analysis}</p></div>}
              {passage.exam_points && <div><div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">考点综述</div><p>{passage.exam_points}</p></div>}
            </div>
          )}
        </section>
      )}
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-border bg-background/95 px-4 py-3 backdrop-blur lg:bottom-0">
        <div className="mx-auto flex max-w-3xl items-center gap-2">
          {!submitted ? (
            <>
              <Link to="/gaokao/cloze" className="rounded-xl border border-border px-3 py-2.5 text-sm">取消</Link>
              <button
                onClick={handleSubmit}
                disabled={submitting}
