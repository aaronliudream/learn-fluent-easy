  ...
  if (!cur && activeSession.length === 0) {
    return (
      <div className="rounded-2xl border border-border/60 bg-card p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground"><T>单词不足 4 个，无法生成测验</T></p>
      </div>);
    ...
    return (
      <div className="rounded-3xl border border-border/60 bg-card p-6 text-center">
        ...
        <p className="mt-1 text-base text-muted-foreground">
          <T>答对</T> {score.correct} / {score.total} ({pct}%)
        </p>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-3">
          <button
            ...
            <RotateCw className="size-4" /> <T>再来</T> {sessionSize} <T>题</T>
          </button>
          <Link
        ...
        <Link
          ...
          <Gamepad2 className="size-3.5" /> <T>或玩个单词游戏 →</T>
        </Link>
      </div>);
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="text-muted-foreground">
            {reviewSession ? "🔁 错题复习 · " : ""}<T>第</T> {idx + 1} / {activeSession.length} <T>题</T>
          </span>
        ...
        <div className="mt-3 flex flex-col items-center gap-2">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span className={cn("font-bold", playsLeft === 0 ? "text-rose-600" : "text-muted-foreground")}>
                <T>🔊 剩</T> {playsLeft} / {LISTEN_MAX_PLAYS} <T>次</T>
              </span>
      ...
      <div className="flex items-center justify-center gap-1 text-[11px] text-muted-foreground">
        <Sparkles className="size-3" /> <T>答题数据已自动接入智能复习系统</T>
      </div>
    </div>);
