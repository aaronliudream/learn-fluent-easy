...
export default function PrimaryReading() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-6">
      ...
      {loading ?
      <div className="py-20 text-center text-muted-foreground"><T>加载中…</T></div> :
      list.length === 0 ?
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-3 text-[11px] opacity-90">
                      <span className="inline-flex items-center gap-1"><Clock className="size-3" /> {a.estimated_minutes} <T>分钟</T></span>
                      <span className="inline-flex items-center gap-0.5">
                        {Array.from({ length: 3 }).map((_, i) =>
                      <Star key={i} className={`size-3 ${i < stars ? "fill-yellow-300 stroke-yellow-200" : "stroke-white/50"}`} />
                      )}
                      </span>
                      {done && <span className="rounded-full bg-white/30 px-2 py-0.5 text-[10px] font-bold"><T>✓ 已完成</T></span>}
                    </div>
                  </div>
