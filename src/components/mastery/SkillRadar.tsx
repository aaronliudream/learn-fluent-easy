  ...
  if (!hasAny) {
    return (
      <div className="rounded-3xl border border-dashed border-border bg-muted/20 p-6 text-center">
        ...
        <div className="mt-3 text-sm font-bold"><T>AI 能力雷达图</T></div>
  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-5">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground"><T>AI · 能力雷达</T></div>
          <h3 className="mt-0.5 text-base font-extrabold"><T>你的 7 维英语能力地图</T></h3>
        </div>
      </div>
      ...
      <div className="mt-3 rounded-2xl bg-gradient-to-br from-rose-50 to-orange-50 p-4 dark:from-rose-500/10 dark:to-orange-500/10">
          ...
          <li key={w.key} className="flex items-center justify-between text-sm">
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-muted-foreground">/ {w.total} <T>项</T></span>
                  <Link
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
        <span className="inline-flex items-center gap-1">
          <span className="size-2.5 rounded-sm bg-primary/60" /> <T>掌握度 = 已掌握 + ½ 熟练</T>
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="size-2.5 rounded-sm" style={{ background: "hsl(142 76% 45%)" }} /> <T>正确率 = 累计答对 / 答过</T>
        </span>
      </div>
    </section>);
