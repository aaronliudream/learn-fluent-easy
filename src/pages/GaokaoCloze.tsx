...
export default function GaokaoCloze() {
    ...
    (async () => {
      ...
      if (user) {
        ...
        (sess || []).forEach((s: any) => {
          ...
          if (!cur) map[s.passage_id] = { passage_id: s.passage_id, best_pct: pct, attempts: 1 };else
          {cur.attempts++;cur.best_pct = Math.max(cur.best_pct, pct);}
        });
        setStats(map);
  ...
  const { active, dueReview, mastered } = useMemo(() => {
    ...
    for (const p of list) {
      ...
      if (row && row.stars >= 5 && !needsReview(row)) m.push(p);else
      if (needsReview(row)) d.push(p);else
      a.push(p);
    }
        ...
        <div className="flex items-start justify-between gap-3 pl-2">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1"><Target className="size-3.5" />{p.blank_count} <T>空</T></span>
              <span className="inline-flex items-center gap-1"><Clock className="size-3.5" />{p.recommended_minutes} <T>分钟</T></span>
              <span className="inline-flex items-center gap-1"><BookOpenCheck className="size-3.5" />{p.word_count || "—"} <T>词</T></span>
              {masteryStatus === "review_due" && <span className="text-amber-600 font-bold"><T>· 该复习了</T></span>}
              {masteryStatus === "mastered" && <span className="text-emerald-600 font-bold"><T>· 完美掌握</T></span>}
              {!unlocked && <span className="text-orange-500 font-bold"><T>· 需先完成上一篇</T></span>}
            </div>
          </div>
          <div className="flex flex-col items-end gap-2">
            ...
            <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[11px] text-primary"><T>未做</T></span>
            }
    ...
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-6">
      ...
      <Link
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold"><T>我的错题本</T></div>
      ...
      <>
        ...
        <section className="mb-4 rounded-2xl border-2 border-amber-400/50 bg-gradient-to-r from-amber-500/10 to-orange-500/10 p-4">
              <div className="mb-2 flex items-center gap-2"><Clock className="size-4 text-amber-600" /><h2 className="text-sm font-extrabold text-amber-700 dark:text-amber-400"><T>⏰ 该复习了 (</T>{dueReview.length})</h2></div>
        ...
        <section className="mt-6">
          ...
          <div className="mt-6 text-[11px] text-muted-foreground leading-5">
            💡 <b><T>解锁规则</T></b><T>：≥80% 通过解锁下一篇 · 100% 升一星 · 5⭐ 完美掌握</T><br />
            🔁 <b><T>遗忘曲线</T></b><T>：1天 → 3天 → 7天 → 14天 → 30天 → 90天</T>
          </div>
        </>
      }
    </main>);
