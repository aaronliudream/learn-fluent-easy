// 生成选择性必修三情景闯关题(context_questions, grade=12, volume='elective3'):从本地 vocab JSON 真实例句挖空。
...
for (const u of Object.keys(META)) {
  const m = META[u];
  const vf = JSON.parse(readFileSync(`scripts/senior-rebuild/elective3/${u}/${VOL}-${u}-vocab.json`, 'utf8'));
...
let sql = '-- 选择性必修三 情景闯关题(context_questions, grade=12),由本地 vocab 真实例句挖空生成。Aaron 跑。\n';
sql += '-- ContextQuiz 高中按 grade=10 + volume 读;写 volume/unit 防串库。幂等(只删本册)。\nBEGIN;\n';
sql += "DELETE FROM public.context_questions WHERE volume='elective3';\n";
for (const r of rows) {
  sql += `INSERT INTO public.context_questions (grade, volume, unit, word, sentence, options, answer) VALUES (12, 'elective3', '${esc(r.unit)}', '${esc(r.word)}', '${esc(r.sentence)}', '${esc(JSON.stringify(r.options))}'::jsonb, '${esc(r.answer)}');\n`;
}
sql += "\nCOMMIT;\nSELECT unit, count(*) FROM public.context_questions WHERE volume='elective3' GROUP BY unit ORDER BY unit;\n";
sql += "SELECT count(*) AS context_q_elective3 FROM public.context_questions WHERE volume='elective3';\n";
writeFileSync('SQLAA/elective3-context-questions-load.sql', sql);
console.log('生成情景题:', rows.length, '| 跳过:', skip, '| -> SQLAA/elective3-context-questions-load.sql');
let bad = 0;
