      ...
      <div className="grid gap-4 md:grid-cols-2">
        {BRANDS.map((b) => (
          <Link
            key={b.to}
            to={b.to}
            className={`group relative flex min-h-[180px] flex-col justify-between overflow-hidden rounded-3xl bg-gradient-to-br ${b.gradient} p-6 text-white shadow-tile transition-all hover:-translate-y-0.5 hover:shadow-[0_24px_50px_-20px_rgba(0,0,0,0.45)]`}
          >
            ...
            <div className="relative mt-4">
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-white/20 px-3 py-1.5 text-[11px] font-bold backdrop-blur-sm transition group-hover:bg-white/30">
                  进入 <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-1" />
                </span>
              </div>
            </div>
          </Link>
