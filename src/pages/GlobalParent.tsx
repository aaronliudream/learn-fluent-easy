...
import {
  ...
  TrendingUp, AlertTriangle, Clock, Flame, Target, Sparkles, Download } from
"lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { cn } from "@/lib/utils";
import {
  ResponsiveContainer, LineChart, Line, XAxis, YAxis, Tooltip, CartesianGrid } from
"recharts";
      ...
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {mainSeg ? <><T>主修</T>：<b><T>{SEG_META[mainSeg].label}</T></b> · </> : null}
                ...
                return (
                  ...
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      <T>{M.label}</T> {seg === mainSeg && <span className="rounded bg-amber-100 px-1.5 text-[10px] text-amber-700"><T>主修</T></span>}
                    </span>
...
}: {seg: "primary" | "junior" | "gaokao";words: Words;extra: {k: string;v: string;}[];}) {
  const M = SEG_META[seg];
  const t = useT();
  return (
