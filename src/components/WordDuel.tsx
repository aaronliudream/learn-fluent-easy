...
export default function WordDuel({
  ...
  const finalize = async (allLogs: RoundLog[]) => {
    ...
    // Coin reward
    if (won) await awardCoins(50);else
    if (draw) await awardCoins(20);else
    await awardCoins(10);
...
function SignInGate() {
  return (
    <div className="rounded-3xl border-2 border-rose-500/40 bg-gradient-to-br from-rose-500/15 via-orange-500/10 to-transparent p-6 shadow-tile text-center">
      ...
      <h2 className="mt-4 text-xl font-extrabold"><T>登录后开战 ⚔️</T></h2>
      <p className="mt-2 text-sm text-muted-foreground">
        <T>Word Duel 需要登录才能匹配对手、记录 ELO 段位与排行榜成绩。</T>
      </p>
      <Link to="/auth" className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-rose-500 px-6 py-3 text-sm font-bold text-white shadow-lg hover:bg-rose-600">
        <LogIn className="size-4" /> <T>立即登录 / 注册</T>
      </Link>
    </div>);
...
}: {me: RatingInfo | null;onMatch: () => void;onBot: () => void;}) {
  return (
    <div>
      <div className="rounded-3xl border-2 border-rose-500/40 bg-gradient-to-br from-rose-500/15 via-orange-500/10 to-transparent p-6 shadow-tile">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold"><T>⚔️ Word Duel 单词决斗</T></h1>
            <p className="text-xs text-muted-foreground mt-0.5"><T>5 回合 · 8 秒/题 · 速度 + 准确度双重加分 · 实时 ELO 段位</T></p>
          </div>
        </div>
        ...
        <div className="mt-5 rounded-2xl border bg-card/80 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground"><T>我的段位</T></div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Button
          ...
          <Swords className="mr-2 size-5" /> <T>真人匹配</T>
        </Button>
        <Button
          onClick={onBot}
          variant="outline"
          ...
          <Bot className="mr-2 size-5" /> <T>挑战 AI</T>
        </Button>
      </div>
...
function MatchingView({ seconds, onCancel, onBot }: {seconds: number;onCancel: () => void;onBot: () => void;}) {
  return (
    <div className="rounded-3xl border-2 border-rose-500/40 bg-gradient-to-br from-rose-500/10 to-transparent p-8 text-center shadow-tile">
      ...
      <h2 className="mt-4 text-2xl font-extrabold"><T>正在为你匹配对手…</T></h2>
      <p className="mt-1 text-sm text-muted-foreground"><T>已搜索</T> {seconds}<T>s · 范围正在扩大</T></p>
      <div className="mt-6 flex flex-col items-center gap-2">
        <Button onClick={onBot} className="rounded-full bg-foreground px-6 text-background">
          <Bot className="mr-2 size-4" /> <T>等不及？立即挑战 AI</T>
        </Button>
        <button onClick={onCancel} className="text-xs text-muted-foreground hover:text-foreground"><T>取消匹配</T></button>
      </div>
    </div>);
...
}: {me: RatingInfo | null;opponent: Opponent;question: Question;roundIdx: number;totalRounds: number;timeLeft: number;myAnswer: string | null;oppAnswered: boolean;revealed: boolean;logs: RoundLog[];onAnswer: (c: string) => void;}) {
  ...
  return (
    <div>
      ...
      <div className="mt-5 rounded-3xl border-2 bg-card p-6 shadow-tile">
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground"><T>选出对应英文单词</T></div>
        ...
        <div className="mt-4 rounded-xl bg-muted/60 p-3 text-sm">
            <T>正确答案：</T><b>{question.correct}</b>
            <span className="ml-3 text-muted-foreground">
              <T>你 +</T>{logs[logs.length - 1]?.my_score || 0} · {opponent.alias} +{logs[logs.length - 1]?.opp_score || 0}
            </span>
          </div>
        }
      </div>
...
}: {name: string;rating: number;score: number;answered: boolean;isMe?: boolean;isBot?: boolean;}) {
  return (
...
function ResultView({
...
}: {banner: {won: boolean;draw: boolean;delta: number;newRating: number;};opponent: Opponent;me: RatingInfo | null;logs: RoundLog[];onPlayAgain: () => void;onExit: () => void;}) {
  const myTotal = logs.reduce((s, l) => s + l.my_score, 0);
  const oppTotal = logs.reduce((s, l) => s + l.opp_score, 0);
  ...
  const share = () => {
    ...
    if (navigator.share) navigator.share({ text: txt }).catch(() => navigator.clipboard.writeText(txt));else
    ...
    <div>
      <div className={cn("rounded-3xl border-2 bg-gradient-to-br p-6 text-center shadow-tile", tone)}>
        ...
        <div className="mt-1 text-sm text-muted-foreground">
          <T>对手：</T>{opponent.is_bot && <Bot className="inline size-3" />} {opponent.alias} · ELO {opponent.rating}
        </div>
      ...
      <div className="mt-4 rounded-2xl border bg-card p-4">
        <div className="mb-2 text-xs font-bold uppercase tracking-wider text-muted-foreground"><T>逐回合战报</T></div>
  ...
  return (
    <div className="mt-4 rounded-3xl border-2 border-rose-500/30 bg-gradient-to-br from-rose-500/10 to-orange-500/5 p-5 shadow-tile">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-base font-extrabold"><T>段位排行榜</T></h3>
      </div>
      {loading ?
      ...
      rows.length === 0 ?
      <div className="py-6 text-center text-sm text-muted-foreground"><T>还没有人上榜，去打第一场吧！</T></div> :
