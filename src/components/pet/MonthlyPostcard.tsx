    ...
    <div onClick={markRead} className="cursor-pointer rounded-3xl border-2 border-amber-200 bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 p-4 shadow-tile dark:border-amber-900/40 dark:from-amber-950/40 dark:via-orange-950/30 dark:to-rose-950/30">
      <div className="flex items-center justify-between">
        <div className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300">
          📮 本月明信片 {!pc.read_at && <span className="ml-1 rounded-full bg-rose-500 px-1.5 py-0.5 text-[9px] text-white">NEW</span>}
        </div>
        {onTrip && <span className="rounded-full bg-amber-500/20 px-2 py-0.5 text-[10px] font-bold text-amber-800 dark:text-amber-200">出游中 · 至 {pc.trip_end.slice(5)}</span>}
      </div>
      <div className="mt-2 flex items-start gap-3">
        <div className="text-5xl">{pc.destination_emoji}</div>
        <div className="flex-1">
          <div className="text-base font-extrabold">来自 {pc.destination_cn} 的问候</div>
          <p className="mt-1 text-sm leading-relaxed text-foreground/80">{pc.message_cn}</p>
          {onTrip && <p className="mt-2 text-[11px] italic text-amber-700 dark:text-amber-300">休息也是学习的一部分 · 这几天不必赶进度</p>}
        </div>
      </div>
    </div>
  );
