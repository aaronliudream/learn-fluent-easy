// 生成必修三某单元灌库 SQL(junior_* 表)。用法: node _gen_load.mjs --unit u1
// 输出 -> SQLAA/required3-<unit>-load.sql + scripts/。自带 count + 分布校验。幂等。
...
let sql = `-- ============================================================\n-- 必修三 ${UNIT}(${m.title})灌库(junior_* 表,volume='${VOL}' unit='${UNIT}' grade=${GRADE})。幂等。\n`;
...
grammar.points.forEach((p, i) => {
  sql += `INSERT INTO public.junior_grammar_points (code, title, cefr, grade, summary, sort_order, volume, unit, category_id, examples, content_depth) VALUES (`
    + `'${esc(px(p.code))}', '${esc(circ[i] + p.point)}', 'A2', ${GRADE}, '${esc('必修三 ' + UNIT + ' — ' + p.overview)}', ${i + 1}, '${VOL}', '${UNIT}', (SELECT id FROM public.junior_grammar_categories WHERE code='senior' LIMIT 1), '[]'::jsonb, 0);\n`;
});
...
listening.exercises.forEach(e => {
  ...
  sql += `INSERT INTO public.junior_listening_exercises (grade, title, topic, difficulty, transcript, translation_cn, speaker, questions, key_vocab, kind, volume, unit) VALUES (`
    + `${GRADE}, '${esc(e.title)}', '必修三 ${UNIT} · ${esc(e.type || '')}', ${e.kind === 'long' ? 3 : 2}, '${esc(tr)}', '${esc(e.translation_cn)}', '${esc(e.speaker || 'us_female')}', ${jb(qs)}, '[]'::jsonb, '${esc(e.kind)}', '${VOL}', '${UNIT}');\n`;
});
...
writeFileSync(`scripts/required3-${U}-load.sql`, sql);
writeFileSync(`SQLAA/required3-${U}-load.sql`, sql);
const ins = (sql.match(/INSERT INTO/g) || []).length;
console.log(`必修三 ${UNIT} 灌库 SQL -> SQLAA/required3-${U}-load.sql`);
