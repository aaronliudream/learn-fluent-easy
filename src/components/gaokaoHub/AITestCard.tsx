...
import { getUnitProgress, nextAiTestMilestone } from "@/lib/gaokaoHub/progress";
import { getUnitState } from "@/lib/gaokaoHub/storage";
import { AI_TEST_PROGRESS_STEP } from "@/lib/gaokaoHub/types";

export function GaokaoAITestCard() {
  const { grade, state } = useGaokaoHub();
  const nav = useNavigate();
  const base = `/gaokao/hub/${grade}`;

  const unit = findUnit(state.currentUnit);
  const unitP = unit ? getUnitProgress(state, unit.id) : { percent: 0, ratio: 0, completed: 0 };
