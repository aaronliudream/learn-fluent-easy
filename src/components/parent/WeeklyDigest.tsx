    ...
    return (
      <section className="mb-4 rounded-3xl border border-border/60 bg-card p-5">
        <div className="flex items-center justify-center text-muted-foreground">
          <Loader2 className="mr-2 size-5 animate-spin" /> 整理本周报告…
        </div>
      </section>
    );
  }
    ...
    <section className="mb-4 rounded-3xl border border-border/60 bg-card p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-extrabold md:text-lg">📰 本周快报 · G{grade}</h2>
        </div>
      ...
      {isQuiet ? (
        <div className="rounded-2xl border-2 border-dashed border-border bg-secondary/40 p-5 text-center">
          ...
          <p className="mt-1 text-sm font-bold">本周还没有学习记录</p>
          <p className="mt-1 text-xs text-muted-foreground">陪孩子一起开始第一节课吧</p>
          <Link to={`/primary/grade/${grade}`} className="mt-3 inline-flex items-center gap-1 rounded-full bg-violet-600 px-4 py-1.5 text-xs font-extrabold text-white hover:bg-violet-700">
            开始今天的学习 <ChevronRight className="size-3" />
          </Link>
        </div>
      ) : (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-2xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 p-4 dark:border-emerald-900 dark:from-emerald-950/30 dark:to-teal-950/20">
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-extrabold text-emerald-700 dark:text-emerald-300">本周新掌握</span>
              </div>
            ...
            {newlyMastered.length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground">还没新掌握的词，多练几次就上榜！</p>
            )}
          </div>
          ...
          <div className="rounded-2xl border-2 border-rose-200 bg-gradient-to-br from-rose-50 to-orange-50 p-4 dark:border-rose-900 dark:from-rose-950/30 dark:to-orange-950/20">
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-extrabold text-rose-700 dark:text-rose-300">本周薄弱词</span>
              </div>
            ...
            {weakWords.length > 0 ? (
              <>
                ...
                <Link to={`/primary/vocab/${grade}?focus=weak`} className="mt-3 inline-flex w-full items-center justify-center gap-1 rounded-full bg-rose-600 px-3 py-1.5 text-xs font-extrabold text-white hover:bg-rose-700">
                  一键补练 6 题 <ChevronRight className="size-3" />
                </Link>
              </>
            ) : (
              <p className="text-xs text-muted-foreground">本周没有明显薄弱词，太棒了！</p>
            )}
          </div>
          ...
          <div className="rounded-2xl border-2 border-sky-200 bg-gradient-to-br from-sky-50 to-blue-50 p-4 dark:border-sky-900 dark:from-sky-950/30 dark:to-blue-950/20">
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <span className="text-sm font-extrabold text-sky-700 dark:text-sky-300">本周学习时长</span>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-black tabular-nums">{minutesThisWeek}</span>
              <span className="text-sm font-bold text-muted-foreground">/ 60 分钟</span>
            </div>
          ...
          <div className="rounded-2xl border-2 border-violet-200 bg-gradient-to-br from-violet-50 to-fuchsia-50 p-4 dark:border-violet-900 dark:from-violet-950/30 dark:to-fuchsia-950/20">
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <span className="text-sm font-extrabold text-violet-700 dark:text-violet-300">本周完成课程</span>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-black tabular-nums">{lessonsThisWeek}</span>
              <span className="text-sm font-bold text-muted-foreground">节</span>
            </div>
            <p className="mt-2 text-[11px] text-muted-foreground">
            ...
            <Link to={`/primary/grade/${grade}`} className="mt-2 inline-flex items-center gap-1 text-[11px] font-extrabold text-violet-700 hover:underline dark:text-violet-300">
              继续下一节 <ChevronRight className="size-3" />
            </Link>
          </div>
        </div>
      )}
