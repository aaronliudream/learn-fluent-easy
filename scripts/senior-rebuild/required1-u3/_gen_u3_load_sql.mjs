// 生成 必修一 U3 灌库 SQL(junior_* 表,volume='required1' unit='U2' grade=10)。照 WU/U1 同款列。自带 count 校验。
import { readFileSync, writeFileSync } from 'node:fs';
const DIR = 'scripts/senior-rebuild/required1-u3/';
const VOL = 'required1', UNIT = 'U3', GRADE = 10;
const LET = ['A', 'B', 'C', 'D'];
...
const vocab = rd('required1-u3-vocab.json');
const grammar = rd('required1-u3-grammar.json');
const reading = rd('required1-u3-reading.json');
const cloze = rd('required1-u3-cloze.json');
const listening = rd('required1-u3-listening.json');
const writing = rd('required1-u3-writing.json');

const VNOTES = {
  'rd1': [{ word: 'honour', cn: '荣誉' }, { word: 'determination', cn: '决心' }, { word: 'champion', cn: '冠军' }],
  'rd2': [{ word: 'graceful', cn: '优雅的' }, { word: 'strength', cn: '力量;心理素质' }, { word: 'failure', cn: '失败' }],
  'rd3': [{ word: 'slim', cn: '苗条的' }, { word: 'diet', cn: '饮食' }, { word: 'positive', cn: '积极的' }],
  'rd4': [{ word: 'compete', cn: '竞争;参赛' }, { word: 'cheat', cn: '作弊' }, { word: 'stress', cn: '压力' }],
  'rd5': [{ word: 'survey', cn: '调查' }, { word: 'event', cn: '比赛项目;活动' }, { word: 'jog', cn: '慢跑' }],
  'rd6': [{ word: 'sportsmanship', cn: '体育精神' }, { word: 'pretend', cn: '假装' }, { word: 'audience', cn: '观众' }],
};
const TOPIC_CN = { 'rd1': '记叙·排球传奇郎平', 'rd2': '记叙·乔丹的心理素质', 'rd3': '记叙·从减肥到健身', 'rd4': '议论·运动为何重要', 'rd5': '说明·校园运动调查', 'rd6': '议论·体育精神' };

let sql = `-- ============================================================\n-- 必修一 U3(Sports and Fitness)灌库(junior_* 表,volume='${VOL}' unit='${UNIT}' grade=${GRADE})。幂等。\n`;
...
vocab.words.forEach((w, i) => {
  ...
  sql += `INSERT INTO public.junior_vocab (grade, word, pos, phonetic, meaning_cn, example_en, example_cn, theme, freq_rank, star_level, stage, source_type, volume, unit, phrase_en, tip) VALUES (`
    + `${GRADE}, '${esc(w.word)}', '${esc(w.pos)}', '${esc(w.ipa)}', '${esc(w.meaning_cn)}', ${w.example_en ? `'${esc(w.example_en)}'` : 'NULL'}, ${w.example_cn ? `'${esc(w.example_cn)}'` : 'NULL'}, 'Sports and Fitness', ${w.freq_rank ?? i + 1}, 0, 'senior', 'wordlist', '${VOL}', '${UNIT}', '${esc(w.phrase_en)}', ${tip ? `'${esc(tip)}'` : 'NULL'});\n`;
});
...
grammar.points.forEach((p, i) => {
  sql += `INSERT INTO public.junior_grammar_points (code, title, cefr, grade, summary, sort_order, volume, unit, category_id, examples, content_depth) VALUES (`
    + `'${esc(p.code)}', '${esc(circ[i] + p.point)}', 'A2', ${GRADE}, '${esc('必修一 U3 — ' + p.overview)}', ${i + 1}, '${VOL}', '${UNIT}', (SELECT id FROM public.junior_grammar_categories WHERE code='senior' LIMIT 1), '[]'::jsonb, 0);\n`;
});
...
reading.passages.forEach(p => {
  ...
  sql += `INSERT INTO public.junior_reading (grade, title, body, topic, word_count, questions, vocab_notes, difficulty, volume, unit) VALUES (`
    + `${GRADE}, '${esc(p.title)}', '${esc(p.body)}', '${esc(TOPIC_CN[p.code] || 'Sports and Fitness')}', ${wc(p.body)}, ${jb(qs)}, ${jb(VNOTES[p.code] || [])}, 2, '${VOL}', '${UNIT}');\n`;
});
...
listening.exercises.forEach(e => {
  ...
  sql += `INSERT INTO public.junior_listening_exercises (grade, title, topic, difficulty, transcript, translation_cn, speaker, questions, key_vocab, kind, volume, unit) VALUES (`
    + `${GRADE}, '${esc(e.title)}', '必修一 U3 · ${esc(e.type)}', ${e.kind === 'long' ? 3 : 2}, '${esc(tr)}', '${esc(e.translation_cn)}', '${esc(e.speaker)}', ${jb(qs)}, '[]'::jsonb, '${esc(e.kind)}', '${VOL}', '${UNIT}');\n`;
});
...
sql += `UNION ALL SELECT 'writing', count(*), 1 FROM public.junior_writing_prompts WHERE volume='${VOL}' AND unit='${UNIT}' ORDER BY k;\n`;
...
writeFileSync('scripts/required1-u3-load.sql', sql);
writeFileSync('SQLAA/required1-u3-load.sql', sql);
console.log('U2 灌库 SQL -> scripts/ + SQLAA/required1-u3-load.sql');
