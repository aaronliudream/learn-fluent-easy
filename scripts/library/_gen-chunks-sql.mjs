// 通用图书馆语块生成器:吃 subagent 产的 chunk-data/ch<N>.json,硬校验 + 跨章去重 + 出 SQL + 审稿 md。
// 用法:node scripts/library/_gen-chunks-sql.mjs <chapter>
// 数据契约(chunk-data/ch<N>.json):
...
if (!CH || CH < 1) { console.error("用法: node scripts/library/_gen-chunks-sql.mjs <chapter>"); process.exit(1); }
const KEY = "tom-sawyer";
const dataPath = `scripts/library/books/chunk-data/ch${CH}.json`;
...
-- 与 REVIEWAA/图书馆词表/tom-sawyer-chunks-ch${CH}-review.md 一致。跨章去重:ch1..${CH-1} 已建卡不重出(仅补索引)。
...
// ---- 审稿 md ----
const md = `# 汤姆·索亚历险记 · 第 ${CH} 章 语块(chunk)候选 — 待审

> **CC 亲判亲抽**(不走 Gemini,配额 429)。判据同 ch1/ch2:可迁移才收、例句另造禁抄原文、逐词 literal、可分式 card-only、跨章去重。
> 产物:\`SQLAA/library-chunks-tom-sawyer-ch${CH}.sql\`(${chunks.length} 条语块 / ${cardRows.length} 卡 / ${idxRows.length} 索引行,幂等 upsert,**未跑**)。审:①边界(该收/误收)②释义+例句(讲反没/简单没/没抄原文)。
...
3. 审过 → Aaron 跑 \`SQLAA/library-chunks-tom-sawyer-ch${CH}.sql\`(幂等,无需重部署 edge)→ :8080 第 ${CH} 章验珊瑚虚线+逐词节。

> 边界:只产文件+SQL(Aaron 跑);未落库、未改读路径/收藏、未合 main。SQL 只有 Aaron 手动跑。
`;
mkdirSync("REVIEWAA/图书馆词表", { recursive: true });
writeFileSync(`REVIEWAA/图书馆词表/tom-sawyer-chunks-ch${CH}-review.md`, md);

console.log(`✓ ch${CH}: ${chunks.length} 条 / ${cardRows.length} 卡(去重${dedupCards}) / ${idxRows.length} 索引 / card-only ${cardOnlyN}`);
console.log(`  → SQLAA/library-chunks-${KEY}-ch${CH}.sql + REVIEWAA/图书馆词表/tom-sawyer-chunks-ch${CH}-review.md`);
