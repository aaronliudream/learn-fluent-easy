...
CREATE TABLE vocab_banks(id uuid PRIMARY KEY DEFAULT gen_random_uuid(), code text UNIQUE NOT NULL,
  total_words int NOT NULL DEFAULT 0);   -- 前端拿它当进度条分母
...
/* ── ① 正常顺序:四份全绿 ────────────────────────────────── */
{
  ...
  console.log('  库内终态:', JSON.stringify(await q(
    `SELECT (SELECT count(*) FROM vocab_words) w,
            ...
            (SELECT total_words FROM vocab_banks WHERE code='gaokao') gk_meta,
            (SELECT total_words FROM vocab_banks WHERE code='zhongkao') zk_meta`)));
  await db.close();
}
...
const MUT = [
  ...
  ['挂载 SQL 不同步 total_words → 应炸「与实挂对不上」', async db => {
    await db.exec(read(FILES[0])); await db.exec(read(FILES[1]));
    /* 把那条 UPDATE vocab_banks 整段摘掉,模拟第一版漏写的样子 */
    const cut = read(FILES[2]).replace(new RegExp('UPDATE vocab_banks b[\\s\\S]*?;' + String.fromCharCode(10)), '');
    if (cut === read(FILES[2])) throw new Error('变异没生效:没找到那段 UPDATE');
    await db.exec(cut);
  }],
...
/* ── ③ 音标修正 SQL:每一轮都真跑 + 变异 ─────────────────── */
const FIX_FILES = ['vocab_fix_ipa_placeholders.sql', 'vocab_fix_ipa_oneself.sql'];

/** 解析某份修正 SQL 的名单 */
function fixRows(file) {
  const m = /INSERT INTO _fix\(id, headword, old_ipa, new_ipa\) VALUES\n([\s\S]*?);\n/.exec(read(file));
  if (!m) throw new Error(`${file} 里没找到名单`);
  const rows = [...m[1].matchAll(/\('([0-9a-f-]{36})', '((?:[^']|'')*)', '((?:[^']|'')*)', '((?:[^']|'')*)'\)/g)]
    .map(([, id, hw, o, n]) => ({ id, hw: hw.replace(/''/g, "'"), old: o.replace(/''/g, "'"), neu: n.replace(/''/g, "'") }));
  if (!rows.length) throw new Error(`${file} 名单一行都没解析出来`);
  return rows;
}
/** 铺前置状态:按名单的 id 和**改前值**建行,再加两个无关词当陪衬。
    ⚠️ 陪衬行不是装饰 —— 没有它,「全库只有这 N 行 ipa 变了」那条断言是真空的。 */
async function seedForFix(file) {
  const db = await fresh();
  const rows = fixRows(file);
  for (const r of rows) {
    await db.query('INSERT INTO vocab_words(id, headword, ipa, def_zh) VALUES ($1,$2,$3,$4)',
      [r.id, r.hw, r.old, '占位释义']);
  }
  await db.query("INSERT INTO vocab_words(headword, ipa, def_zh) VALUES ('subject','/ˈsʌb.dʒɪkt/','主题'),('oneself','/wʌnˈsɛlf/','自己')");
  return { db, rows };
}

for (const FIX of FIX_FILES) {
  console.log(`\n══ ${FIX} ══`);
  {
    const { db, rows } = await seedForFix(FIX);
    try { await db.exec(read(FIX)); console.log(`  ✓ 真跑通过(名单 ${rows.length} 条)`); }
    catch (e) { console.log(`  ✗ ${e.message}`); process.exitCode = 1; }
    const bad = (await db.query(
      `SELECT count(*) FILTER (WHERE headword NOT IN ('subject','oneself') AND (
                ipa LIKE '%sʌm%' OR ipa LIKE '%sɪb%' OR ipa LIKE '%sbz%' OR ipa LIKE '%(%')) AS still_bad,
              count(*) AS total FROM vocab_words`)).rows[0];
    console.log('  库内终态:', JSON.stringify(bad), '← still_bad 应为 0,陪衬的 subject/oneself 必须原样还在');
    await db.close();
  }

  const MUT = [
    ['库里的值已经被人改过 → 应炸「改前值对不上」', async (db, rows) => {
      await db.query('UPDATE vocab_words SET ipa = $1 WHERE id = $2', ['/whatever/', rows[0].id]);
      await db.exec(read(FIX));
    }],
    ['名单里的某个 id 在库里不存在 → 应炸「id 不存在」', async (db, rows) => {
      await db.query('DELETE FROM vocab_words WHERE id = $1', [rows[0].id]);
      await db.exec(read(FIX));
    }],
    ...
    ['SQL 顺手多改了一列(def_zh)→ 应炸「其它列被改动」', async db => {
      await db.exec(read(FIX).replace('DO $gate$',
        "UPDATE vocab_words SET def_zh = '被顺手改了' FROM _fix f WHERE vocab_words.id = f.id;" + String.fromCharCode(10) + 'DO $gate$'));
    }],
    ['SQL 越界改了名单之外的行 → 应炸「全库 ipa 变化行数」', async db => {
      await db.exec(read(FIX).replace('DO $gate$',
        "UPDATE vocab_words SET ipa = '/x/' WHERE headword = 'subject';" + String.fromCharCode(10) + 'DO $gate$'));
    }],
  ];
  for (const [name, run] of MUT) {
    const { db, rows } = await seedForFix(FIX);
    let caught = null;
    try { await run(db, rows); } catch (e) { caught = e.message.split('\n')[0]; }
    console.log(`  ${caught ? '✓ 拦下' : '✗ 没拦下'}  ${name}${caught ? `\n           → ${caught.slice(0, 100)}` : ''}`);
    if (!caught) process.exitCode = 1;
    await db.close();
  }
