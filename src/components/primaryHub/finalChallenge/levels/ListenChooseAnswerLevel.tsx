/**
 * 听句选答 (listen_and_choose_answer) — AI 强化训练专用题型
 *
 * 以「听音辨词」(ListenChooseWordLevel) 为模板:听 audio → 选文本选项。
 * 差异:
 *   - audio 是完整问句/句子 (如 "What's his job?"),options 是完整答语句子
 *   - 选项竖排 (句子比单词长),左对齐
 *   - 题干文案「听一听,选出正确的回答。」
 *
 * 音频解锁用同步 unlockAudioSync(iOS 安全),朗读走 speakKid。
 * 目前仅经 strengthen 训练页通过 ListenChooseAnswerPlayCard 使用(无正式闯关关卡)。
 */
...
type Q = Extract<FCQuestion, { type: "listen_and_choose_answer" }>;
...
export default function ListenChooseAnswerLevel() {
  ...
  return (
    <LevelShell
      levelId={0}
      levelName="听句选答"
      introHint="听一句话,从 3 个回答里选出正确的那个。每句会自动读两遍。"
...
function PlayCard({
  ...
  // 句子较长,安全 fallback 放宽到 3.5s (照 ListenJudgePicture 模式)。
  const playWord = useCallback(
    (text: string): Promise<void> => {
      ...
      return new Promise<void>((resolve) => {
        ...
        const safety = window.setTimeout(done, 3500);
...
/** 暴露给 strengthen 训练页 — 按 type 路由用。 */
export { PlayCard as ListenChooseAnswerPlayCard };
