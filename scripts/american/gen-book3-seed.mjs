// 美语课程 第三册 seed 生成器 —— 读 docs/american/book3/am3_l*.json → SQLAA/american_am3_seed_unitNN.sql
...
// 幂等:全部 ON CONFLICT。按 unit_no 分单元写文件,增课后重跑累积。第三册 5 课/单元(L1-5=U1…L56-60=U12)。
// 用法: node scripts/american/gen-book3-seed.mjs
import { readFileSync, writeFileSync, mkdirSync, readdirSync, existsSync } from "node:fs";
import path from "node:path";

const ROOT = process.cwd();
const SRCDIR = path.join(ROOT, "docs/american/book3");
...
// ---------- 载入所有 am3_l*.json ----------
const files = readdirSync(SRCDIR).filter((f) => /^am3_l\d+\.json$/.test(f)).sort();
if (!files.length) { console.error("docs/american/book3/ 下无 am3_l*.json"); process.exit(1); }
...
for (const U of units) {
  ...
  const hdr = [
    `-- 美语课程 第三册 seed · 单元 ${U}(生成器 gen-book3-seed.mjs 产出,勿手改;改 am3_l*.json 后重跑)`,
    `-- 幂等:全部 ON CONFLICT。依赖 SQLAA/american_add_book_no.sql 已跑(book_no 列)。`,
    `-- 本单元课:${ls.map((L) => L.id).join(", ")}`,
    "BEGIN;",
  ];
  const fp = path.join(ROOT, "SQLAA", `american_am3_seed_unit${p2(U)}.sql`);
...
console.log("\n========== 本次写文件判定(时间戳只在真变时才刷)==========");
if (written.length) for (const U of written) console.log(`  ✍️  需跑  american_am3_seed_unit${p2(U)}.sql(内容有变,已重写)`);
if (unchanged.length) for (const U of unchanged) console.log(`  ⏭️  无需重跑  american_am3_seed_unit${p2(U)}.sql(内容不变,未动文件)`);
