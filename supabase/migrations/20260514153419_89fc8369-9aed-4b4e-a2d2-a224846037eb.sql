  ...
  result := jsonb_build_object(
    ...
    'weak_kps', COALESCE((
      ...
      FROM (
        ...
        JOIN v_gaokao_all_knowledge_points v ON v.id = m.item_id
    ...
    'time_analysis', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'skill_area', skill_area, 'avg_time', avg_time,
        'baseline_time', baseline_time, 'delta_seconds', avg_time - baseline_time, 'count', cnt
      ) ORDER BY ABS(avg_time - baseline_time) DESC)
      FROM (
        SELECT question_type as skill_area,
          ROUND(AVG(time_spent_seconds))::int as avg_time,
          CASE question_type WHEN 'listening' THEN 25 WHEN 'reading' THEN 90
            WHEN 'grammar' THEN 18 WHEN 'cloze' THEN 12 WHEN 'vocab' THEN 6 ELSE 30 END as baseline_time,
          COUNT(*)::int as cnt
        FROM gaokao_user_attempts
        WHERE user_id = p_user_id AND created_at > NOW() - INTERVAL '30 days'
          AND time_spent_seconds IS NOT NULL
        GROUP BY question_type HAVING COUNT(*) >= 3
      ) t
    ), '[]'::jsonb),
    'weakness_personality', '[]'::jsonb,
    'days_to_gaokao', (
      SELECT GREATEST(0, (event_date - CURRENT_DATE))::int
      FROM gaokao_exam_calendar WHERE event_type = 'gaokao'
