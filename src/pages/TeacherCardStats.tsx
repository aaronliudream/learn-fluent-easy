...
export default function TeacherCardStats() {
  ...
  if (unauthorized) {
    return <main className="max-w-md mx-auto p-8 text-center space-y-3">
      <p className="text-muted-foreground">这张卡不是你创建的，无法查看数据。</p>
      <Link to="/teacher/cards"><Button>返回我的卡片</Button></Link>
    </main>;
  }
  if (!card) {
    return <main className="max-w-md mx-auto p-8 text-center space-y-3">
      <p className="text-muted-foreground">卡片不存在</p>
      <Link to="/teacher/cards"><Button>返回</Button></Link>
    </main>;
  }
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      <BackLink to="/teacher/cards">返回卡片列表</BackLink>
      <header>
        <p className="text-xs uppercase tracking-wider text-muted-foreground">卡片数据</p>
        <h1 className="text-xl md:text-2xl font-bold leading-tight mt-1">{card.question}</h1>
        <div className="flex flex-wrap gap-2 mt-2 text-xs text-muted-foreground">
          <Badge variant="secondary">{card.view_count} 扫码</Badge>
          <Badge variant="secondary">{attempts.length} 次答题</Badge>
          <Badge variant="secondary">{totalAnswers} 道答题次数</Badge>
          <Badge variant="secondary">总正确率 {overallPct}%</Badge>
          <Link to={`/q/${card.slug}`} className="underline">查看卡片 →</Link>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {stageGroups.map((s) => (
            <Card key={s.stage} className="p-4">
              ...
              <p className="text-xs text-muted-foreground">完成人次</p>
              {s.n > 0 && (
                <div className="mt-2 space-y-1.5">
                  <div className="flex items-center gap-2 text-xs">
                    <Target className="w-3.5 h-3.5 text-primary" />
                    平均正确率 <span className="font-semibold">{s.avg}%</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs">
                    <Trophy className="w-3.5 h-3.5 text-amber-500" />
                    满分 <span className="font-semibold">{s.perfect}</span>
                  </div>
                </div>
              )}
            </Card>
                ...
                <Card key={row.idx} className="p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {row.pct !== null && (
                      <div className={`text-right shrink-0 ${
                        ...
                        <p className="text-[10px] uppercase tracking-wider">正确率</p>
                      </div>
                    )}
                  </div>
                  ...
                  {row.mostWrong >= 0 && row.pct !== null && row.pct < 70 && (
                    <div className="text-xs bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900 rounded-md px-2.5 py-1.5 text-rose-700 dark:text-rose-300">
                      💡 最常错选的是 <strong>{String.fromCharCode(65 + row.mostWrong)}</strong>
                      （{row.mostWrongCount} 次）— 也许讲解里要重点澄清这个误解。
                    </div>
                  )}
                </Card>
              );
