// 上外(sufe)必修一 情景闯关题(context_questions, grade=10, volume='required1', publisher='sufe')。
// 从本地 vocab JSON 真实例句挖空。DELETE 按 publisher+volume(只删 sufe 本册,不动人教)。
...
for (const u of Object.keys(META)) {
  const m = META[u];
  const vf = JSON.parse(readFileSync(`scripts/senior-rebuild/sufe-${VOL}/${u}/${VOL}-${u}-vocab.json`, 'utf8'));
...
let sql = `-- 上外(sufe)必修一 情景闯关题(context_questions, grade=${GRADE}, volume='${VOL}', publisher='${PUB}')。Aaron 跑。幂等(只删 sufe 本册)。\nBEGIN;\n`;
...
sql += `SELECT count(*) AS context_q_sufe_${VOL} FROM public.context_questions WHERE publisher='${PUB}' AND volume='${VOL}';\n`;
writeFileSync(`SQLAA/sufe-${VOL}-context-questions-load.sql`, sql);
console.log('生成情景题:', rows.length, '| 跳过:', skip, `| -> SQLAA/sufe-${VOL}-context-questions-load.sql`);
let bad = 0;
