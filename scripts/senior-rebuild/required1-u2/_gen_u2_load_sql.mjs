// 生成 必修一 U2 灌库 SQL(junior_* 表,volume='required1' unit='U2' grade=10)。照 WU/U1 同款列。自带 count 校验。
import { readFileSync, writeFileSync } from 'node:fs';
const DIR = 'scripts/senior-rebuild/required1-u2/';
const VOL = 'required1', UNIT = 'U2', GRADE = 10;
const LET = ['A', 'B', 'C', 'D'];
...
const vocab = rd('required1-u2-vocab.json');
const grammar = rd('required1-u2-grammar.json');
const reading = rd('required1-u2-reading.json');
const cloze = rd('required1-u2-cloze.json');
const listening = rd('required1-u2-listening.json');
const writing = rd('required1-u2-writing.json');

const VNOTES = {
  'rd1': [{ word: 'empire', cn: '帝国' }, { word: 'site', cn: '遗址;地点' }, { word: 'destination', cn: '目的地' }],
  'rd2': [{ word: 'visa', cn: '签证' }, { word: 'accommodation', cn: '住宿' }, { word: 'arrangement', cn: '安排' }],
  'rd3': [{ word: 'narrow', cn: '狭窄的' }, { word: 'admire', cn: '欣赏;钦佩' }, { word: 'steep', cn: '陡峭的' }],
  'rd4': [{ word: 'transport', cn: '交通运输' }, { word: 'rent', cn: '租用' }, { word: 'view', cn: '景色;看法' }],
  'rd5': [{ word: 'castle', cn: '城堡' }, { word: 'architecture', cn: '建筑' }, { word: 'unique', cn: '独特的' }],
  'rd6': [{ word: 'culture', cn: '文化' }, { word: 'experience', cn: '经历;经验' }, { word: 'admire', cn: '欣赏' }],
};
const TOPIC_CN = { 'rd1': '说明·探索秘鲁', 'rd2': '说明·行前规划', 'rd3': '记叙·山中徒步', 'rd4': '说明·出行方式', 'rd5': '说明·山中城堡', 'rd6': '议论·为何旅行' };

let sql = `-- ============================================================\n-- 必修一 U2(Travelling Around)灌库(junior_* 表,volume='${VOL}' unit='${UNIT}' grade=${GRADE})。幂等。\n`;
...
vocab.words.forEach((w, i) => {
  ...
  sql += `INSERT INTO public.junior_vocab (grade, word, pos, phonetic, meaning_cn, example_en, example_cn, theme, freq_rank, star_level, stage, source_type, volume, unit, phrase_en, tip) VALUES (`
    + `${GRADE}, '${esc(w.word)}', '${esc(w.pos)}', '${esc(w.ipa)}', '${esc(w.meaning_cn)}', ${w.example_en ? `'${esc(w.example_en)}'` : 'NULL'}, ${w.example_cn ? `'${esc(w.example_cn)}'` : 'NULL'}, 'Travelling Around', ${w.freq_rank ?? i + 1}, 0, 'senior', 'wordlist', '${VOL}', '${UNIT}', '${esc(w.phrase_en)}', ${tip ? `'${esc(tip)}'` : 'NULL'});\n`;
});
...
grammar.points.forEach((p, i) => {
  sql += `INSERT INTO public.junior_grammar_points (code, title, cefr, grade, summary, sort_order, volume, unit, category_id, examples, content_depth) VALUES (`
    + `'${esc(p.code)}', '${esc(circ[i] + p.point)}', 'A2', ${GRADE}, '${esc('必修一 U2 — ' + p.overview)}', ${i + 1}, '${VOL}', '${UNIT}', (SELECT id FROM public.junior_grammar_categories WHERE code='senior' LIMIT 1), '[]'::jsonb, 0);\n`;
});
...
reading.passages.forEach(p => {
  ...
  sql += `INSERT INTO public.junior_reading (grade, title, body, topic, word_count, questions, vocab_notes, difficulty, volume, unit) VALUES (`
    + `${GRADE}, '${esc(p.title)}', '${esc(p.body)}', '${esc(TOPIC_CN[p.code] || 'Travelling Around')}', ${wc(p.body)}, ${jb(qs)}, ${jb(VNOTES[p.code] || [])}, 2, '${VOL}', '${UNIT}');\n`;
});
...
listening.exercises.forEach(e => {
  ...
  sql += `INSERT INTO public.junior_listening_exercises (grade, title, topic, difficulty, transcript, translation_cn, speaker, questions, key_vocab, kind, volume, unit) VALUES (`
    + `${GRADE}, '${esc(e.title)}', '必修一 U2 · ${esc(e.type)}', ${e.kind === 'long' ? 3 : 2}, '${esc(tr)}', '${esc(e.translation_cn)}', '${esc(e.speaker)}', ${jb(qs)}, '[]'::jsonb, '${esc(e.kind)}', '${VOL}', '${UNIT}');\n`;
});
...
writeFileSync('scripts/required1-u2-load.sql', sql);
writeFileSync('SQLAA/required1-u2-load.sql', sql);
console.log('U2 灌库 SQL -> scripts/ + SQLAA/required1-u2-load.sql');
