    ...
    return (
      <main className="mx-auto min-h-screen max-w-2xl px-5 py-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Loader2 className="size-4 animate-spin" /> <T>加载掌握度数据…</T>
        </div>
      </main>);
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl px-5 py-8">
      ...
      <header className="mb-6">
        <h1 className="text-3xl font-extrabold tracking-tight"><T>📊 掌握度仪表盘</T></h1>
        <p className="mt-1 text-sm text-muted-foreground">
          <T>基于 Nation 词汇知识模型 + FSRS-4.5 复习算法的科学评估</T>
        </p>
      </header>
      ...
      <section className="mt-6 rounded-2xl border bg-card p-5">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-base font-bold"><T>5 级掌握度分布</T></h2>
          <span className="text-xs text-muted-foreground"><T>共</T> {totalForBar} <T>词</T></span>
        </div>
      ...
      <section className="mt-4 rounded-2xl border bg-card p-5">
        <h2 className="mb-1 text-base font-bold"><T>三维能力（已学单词均值）</T></h2>
        <p className="mb-3 text-xs text-muted-foreground">
          <T>Form 形（拼写/听辨）· Meaning 义（中英互译/同义/释义）· Use 用（搭配/词性）</T>
        </p>
        <div className="grid grid-cols-3 gap-3">
      ...
      {lapsesTop.length > 0 &&
      <section className="mt-4 rounded-2xl border bg-card p-5">
          <h2 className="mb-3 text-base font-bold"><T>⚠️ 易错词 Top 10</T></h2>
      ...
      <div className="mt-8 rounded-2xl border-2 border-dashed border-muted bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
          <T>先去学一组单词吧，数据会自动出现在这里 ✨</T>
        </div>
      }

      <div className="mt-6 flex justify-end">
        <Button variant="outline" onClick={onExit}><T>返回</T></Button>
      </div>
    </main>);
