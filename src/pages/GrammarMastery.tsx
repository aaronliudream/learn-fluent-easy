...
function StageView({ stage }: {stage: Stage;}) {
  ...
  const stats = useMemo(() => {
    ...
    for (const p of points) {
      ...
      if (b === "expert" || b === "mastered") mastered++;else
      if (b === "ok") learning++;else
      if (b === "weak") weak++;
    }
  ...
  if (!signedIn) {
    return (
      <div className="rounded-2xl border border-border bg-card p-6 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground"><T>登录后即可查看你的语法掌握全景</T></p>
  ...
  if (loading) return <div className="rounded-2xl border border-border bg-card p-8 text-center text-sm text-muted-foreground"><T>加载中…</T></div>;

  if (points.length === 0) {
    return (
      <div className="rounded-2xl border border-border bg-card p-6 text-center text-sm text-muted-foreground">
        <T>该学段尚未配置语法考点（敬请期待）。</T>
      </div>);
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-border bg-card p-4">
        <h3 className="mb-3 text-sm font-bold"><T>掌握度分布</T></h3>
      ...
      <section className="flex flex-wrap gap-2">
        ...
        ["weak", `⚠️ 需加强 (${stats.weak + Math.max(0, stats.total - stats.mastered - stats.learning - stats.weak)})`]] as
        [Filter, string][]).map(([k, label]) =>
        <button
          key={k}
          onClick={() => setFilter(k)}
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {p.grade} <T>· 已练</T> {p.attempts} <T>题 · 正确</T> {p.correct}
                      </div>
                    </div>
      ...
      <Link
        ...
        <div className="flex-1">
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-85"><T>📍 去练一练</T></div>
          <div className="text-base font-extrabold"><T>虚拟语气全攻克</T></div>
        </div>
