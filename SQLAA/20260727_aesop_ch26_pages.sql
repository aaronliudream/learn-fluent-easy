-- ============================================================================
-- 图书馆「绘本模式」· 伊索寓言 ch26《螃蟹与蟹妈妈》分页(3 页 / 3 张图)
--
-- ⚠️ 抗切分写法(ch8 起的默认约定):不用任何 do $$ 块、注释里不留分号、
--    断言用 CASE + 强制类型转换。
--
-- 分页口径:本章只有 1 段,按 seq 升序 row_number() 硬切 1/1/1,para_idx 一个字不动。
--    页1(句1)= 蟹妈妈教训儿子「你为什么老横着走 该直着走」
--    页2(句2)= 小蟹「妈妈你走给我看 你怎么走我就怎么走」
--    页3(句3)= 她试了一回又一回 自己也做不到
--
-- ✅ 3 张图已传桶(2026-07-27,1200 宽 q82),公开地址实测全部 HTTP 200
--    library-illustrations/aesop-easy-readers/ch26/p{1,2,3}.jpg
--
-- 画风:卡通彩色。
--
-- 画面验收要点:
--   ① 笑点全在第 3 页 —— 蟹妈妈自己也只能横着挪,沙上留下一串弯折的脚印作为证据。
--   ② 小蟹在第 3 页只是站着看,不能也在走(否则读不出谁在示范)。
--   ③ 两只蟹全程分开、不接触(防主体糊在一起)。
--   ④ 用体型区分角色:妈妈明显大、小蟹明显小,三页一致。
--
...
select count(*) as ch26_rows,
       count(distinct s.para_idx) as ch26_paras,
       count(s.page_index) as ch26_paged_before
from public.library_sentences s
join public.library_books b on b.id = s.book_id
where b.book_key = 'aesop-easy-readers'
  and s.chapter_idx = 26;

-- 🔒 前置硬闸:句数必须恰好 3
select case
         when count(*) = 3
           then 'OK 前置 ch26 恰好 3 句 硬编码切点 1/2/3 成立'
         else ('前置硬闸失败 ch26 句数=' || count(*) ||
...
select count(distinct s.para_idx) as ch26_paras_after
from public.library_sentences s
join public.library_books b on b.id = s.book_id
where b.book_key = 'aesop-easy-readers'
  and s.chapter_idx = 26;
...
-- ch1-ch25 未被本单影响自检:期望 25 章各 3 页 3 图
select count(*) as chapters_1_to_25_ok
from (
  select s.chapter_idx
  from public.library_sentences s
  join public.library_books b on b.id = s.book_id
  where b.book_key = 'aesop-easy-readers'
    and s.chapter_idx between 1 and 25
    and s.page_index is not null
