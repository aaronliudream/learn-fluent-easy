// 生成 必修一 U5 灌库 SQL(junior_* 表,volume='required1' unit='U2' grade=10)。照 WU/U1 同款列。自带 count 校验。
import { readFileSync, writeFileSync } from 'node:fs';
const DIR = 'scripts/senior-rebuild/required1-u5/';
const VOL = 'required1', UNIT = 'U5', GRADE = 10;
const LET = ['A', 'B', 'C', 'D'];
...
const vocab = rd('required1-u5-vocab.json');
const grammar = rd('required1-u5-grammar.json');
const reading = rd('required1-u5-reading.json');
const cloze = rd('required1-u5-cloze.json');
const listening = rd('required1-u5-listening.json');
const writing = rd('required1-u5-writing.json');

const VNOTES = {
  'rd1': [{ word: 'civilisation', cn: '文明' }, { word: 'system', cn: '体系;系统' }, { word: 'unify', cn: '统一' }],
  'rd2': [{ word: 'calligraphy', cn: '书法' }, { word: 'classic', cn: '经典的' }, { word: 'appreciate', cn: '欣赏' }],
  'rd3': [{ word: 'native', cn: '本地的;母语的' }, { word: 'billion', cn: '十亿' }, { word: 'point of view', cn: '观点' }],
  'rd4': [{ word: 'struggle', cn: '艰难;奋斗' }, { word: 'tongue', cn: '舌头;语言' }, { word: 'attitude', cn: '态度' }],
  'rd5': [{ word: 'dialect', cn: '方言' }, { word: 'apartment', cn: '公寓' }, { word: 'polite', cn: '礼貌的' }],
  'rd6': [{ word: 'advice', cn: '建议' }, { word: 'trouble', cn: '困难' }, { word: 'depend', cn: '取决于' }],
};
const TOPIC_CN = { 'rd1': '说明·汉字书写系统', 'rd2': '说明·中国书法', 'rd3': '议论·为何学外语', 'rd4': '记叙·学德语的奋斗', 'rd5': '说明·英美英语差异', 'rd6': '应用·学英语困难与建议' };

let sql = `-- ============================================================\n-- 必修一 U5(Languages Around the World)灌库(junior_* 表,volume='${VOL}' unit='${UNIT}' grade=${GRADE})。幂等。\n`;
...
vocab.words.forEach((w, i) => {
  ...
  sql += `INSERT INTO public.junior_vocab (grade, word, pos, phonetic, meaning_cn, example_en, example_cn, theme, freq_rank, star_level, stage, source_type, volume, unit, phrase_en, tip) VALUES (`
    + `${GRADE}, '${esc(w.word)}', '${esc(w.pos)}', '${esc(w.ipa)}', '${esc(w.meaning_cn)}', ${w.example_en ? `'${esc(w.example_en)}'` : 'NULL'}, ${w.example_cn ? `'${esc(w.example_cn)}'` : 'NULL'}, 'Languages Around the World', ${w.freq_rank ?? i + 1}, 0, 'senior', 'wordlist', '${VOL}', '${UNIT}', '${esc(w.phrase_en)}', ${tip ? `'${esc(tip)}'` : 'NULL'});\n`;
});
...
grammar.points.forEach((p, i) => {
  sql += `INSERT INTO public.junior_grammar_points (code, title, cefr, grade, summary, sort_order, volume, unit, category_id, examples, content_depth) VALUES (`
    + `'${esc(p.code)}', '${esc(circ[i] + p.point)}', 'A2', ${GRADE}, '${esc('必修一 U5 — ' + p.overview)}', ${i + 1}, '${VOL}', '${UNIT}', (SELECT id FROM public.junior_grammar_categories WHERE code='senior' LIMIT 1), '[]'::jsonb, 0);\n`;
});
...
reading.passages.forEach(p => {
  ...
  sql += `INSERT INTO public.junior_reading (grade, title, body, topic, word_count, questions, vocab_notes, difficulty, volume, unit) VALUES (`
    + `${GRADE}, '${esc(p.title)}', '${esc(p.body)}', '${esc(TOPIC_CN[p.code] || 'Languages Around the World')}', ${wc(p.body)}, ${jb(qs)}, ${jb(VNOTES[p.code] || [])}, 2, '${VOL}', '${UNIT}');\n`;
});
...
listening.exercises.forEach(e => {
  ...
  sql += `INSERT INTO public.junior_listening_exercises (grade, title, topic, difficulty, transcript, translation_cn, speaker, questions, key_vocab, kind, volume, unit) VALUES (`
    + `${GRADE}, '${esc(e.title)}', '必修一 U5 · ${esc(e.type)}', ${e.kind === 'long' ? 3 : 2}, '${esc(tr)}', '${esc(e.translation_cn)}', '${esc(e.speaker)}', ${jb(qs)}, '[]'::jsonb, '${esc(e.kind)}', '${VOL}', '${UNIT}');\n`;
});
...
const tips = JSON.parse(readFileSync(DIR + 'required1-u5-grammar-tips.json', 'utf8'));
...
writeFileSync('scripts/required1-u5-load.sql', sql);
writeFileSync('SQLAA/required1-u5-load.sql', sql);
console.log('U2 灌库 SQL -> scripts/ + SQLAA/required1-u5-load.sql');
