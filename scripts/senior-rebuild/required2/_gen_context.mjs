// 生成必修二情景闯关题(context_questions, grade=10, volume='required2'):从本地 vocab JSON 真实例句挖空。
...
for (const u of Object.keys(META)) {
  const m = META[u];
  const vf = JSON.parse(readFileSync(`scripts/senior-rebuild/required2/${u}/${VOL}-${u}-vocab.json`, 'utf8'));
...
let sql = '-- 必修二 情景闯关题(context_questions, grade=10),由本地 vocab 真实例句挖空生成。Aaron 跑。\n';
sql += '-- ContextQuiz 高中按 grade=10 + volume 读;写 volume/unit 防串库。幂等(只删本册)。\nBEGIN;\n';
sql += "DELETE FROM public.context_questions WHERE grade=10 AND volume='required2';\n";
for (const r of rows) {
  sql += `INSERT INTO public.context_questions (grade, volume, unit, word, sentence, options, answer) VALUES (10, 'required2', '${esc(r.unit)}', '${esc(r.word)}', '${esc(r.sentence)}', '${esc(JSON.stringify(r.options))}'::jsonb, '${esc(r.answer)}');\n`;
}
sql += "\nCOMMIT;\nSELECT unit, count(*) FROM public.context_questions WHERE grade=10 AND volume='required2' GROUP BY unit ORDER BY unit;\n";
sql += "SELECT count(*) AS context_q_required2 FROM public.context_questions WHERE grade=10 AND volume='required2';\n";
writeFileSync('SQLAA/required2-context-questions-load.sql', sql);
console.log('生成情景题:', rows.length, '| 跳过:', skip, '| -> SQLAA/required2-context-questions-load.sql');
let bad = 0;
