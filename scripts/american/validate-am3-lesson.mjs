/**
 * AM3 单课机器校验套件(自主生产步骤9)。用法: node validate-am3-lesson.mjs am3_lNN
 * 与 validate-am2 完全同规则,仅改:源目录 book3、seed 名 am3、单元大小 5 课/单元(am2 为 8)。
...
const id = process.argv[2]; if(!id){console.error("用法: validate-am3-lesson.mjs am3_lNN");process.exit(1);}
const unit = String(Math.ceil(Number(id.match(/l(\d+)/)[1])/5)).padStart(2,"0");
const J = JSON.parse(readFileSync(`docs/american/book3/${id}.json`,"utf8"));
const exp = readFileSync(`docs/american/book3/${id}_explanations_final.md`,"utf8");
const sql = readFileSync(`SQLAA/american_am3_seed_unit${unit}.sql`,"utf8");
