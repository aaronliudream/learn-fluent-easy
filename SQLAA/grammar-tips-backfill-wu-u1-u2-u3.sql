-- ============================================================
-- 回补/重写 必修一 WU/U1/U2/U3 语法小知识(通用 table {headers,rows} 格式)。
...
-- ===== WU 基本句型(五种基本句型 / 句子成分) =====
INSERT INTO public.junior_grammar_tips (grade, volume, unit, content) VALUES (10, 'required1', 'WU', '{
  "title": "英语基本句型 速查",
  ...
  "examVsReal": [
    { "exam": "It tastes good.（完整主系表）", "real": "Tastes good!（口语省略主语）", "note": "考试要完整句;口语常省略主谓" }
  ]
}'::jsonb);

-- ===== U1 名词短语 / 形容词短语 / 副词短语 =====
INSERT INTO public.junior_grammar_tips (grade, volume, unit, content) VALUES (10, 'required1', 'U1', '{
  ...
  "examVsReal": [
    { "exam": "a big old wooden house（按形容词顺序）", "real": "口语里顺序没那么死", "note": "考试按 OSASCOMP;日常交流不严格" }
  ]
}'::jsonb);

-- ===== U2 谈论将来:be going to / will / 现在进行时表将来 =====
INSERT INTO public.junior_grammar_tips (grade, volume, unit, content) VALUES (10, 'required1', 'U2', '{
  "title": "将来时三式 速查",
  ...
  "examVsReal": [
    { "exam": "I am leaving tomorrow.（已定）", "real": "I will leave tomorrow.（口语也常用)", "note": "考试看语境区分;口语 will / be going to 常通用" }
  ]
}'::jsonb);

-- ===== U3 反意疑问句(改成通用 table 格式) =====
...
COMMIT;
-- ===== 自带校验:4 个单元都应有 grammar_tips,各含 table.rows / specialRules / examVsReal =====
SELECT unit,
