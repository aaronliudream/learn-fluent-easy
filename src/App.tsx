...
const Reading = lazy(() => import("./pages/Reading.tsx"));
const ReadingPlay = lazy(() => import("./pages/ReadingPlay.tsx"));
// 图书馆(/library)· 独立顶层 section,与 /reading(P0)完全隔离。
...
const VocabPhonics = lazy(() => import("./pages/vocab/VocabPhonics.tsx"));
    ...
    <TooltipProvider>
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          {/* 阅读中心(全站一级入口,跨学段) */}
          <Route path="/reading" element={<ChineseOnlyRoute><Reading /></ChineseOnlyRoute>} />
          ...
          {/* 音标基础 + 自然拼读(同页二级 tab) */}
          <Route path="/vocab/phonics" element={<ChineseOnlyRoute><VocabPhonics /></ChineseOnlyRoute>} />
          {/* 磨耳朵。静态段,不会被 /vocab/:bankCode 抢走(react-router 按具体度排)。
              ⚠️ 与 /vocab/:bankCode/listen(听音辨义做题)是两个页面,别搞混。 */}
