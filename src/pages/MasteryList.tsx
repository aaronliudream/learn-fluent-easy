  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-6 [font-variant-numeric:tabular-nums]">
      <BackLink to="/dashboard" className="mb-3 inline-flex items-center gap-1 text-sm text-muted-foreground">
        <ArrowLeft className="size-4" /> 返回学习地图
      </BackLink>
      ...
      {rows && rows.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
          <p className="text-sm text-muted-foreground">这一类暂时没有内容 🌿</p>
        </div>
      )}
      ...
      {rows && rows.length > 0 && (
        <div className="space-y-5">
                    ...
                    <li key={r.id} className="flex items-center gap-3 px-3 py-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 text-[11px] text-muted-foreground">
                          {r.attempt_count} 次 · 错 {r.wrong_count}
                          {r.due_at && state === "due" && (
                            <> · 到期 {new Date(r.due_at).toLocaleDateString("zh-CN")}</>
                          )}
                          {r.last_review_at && state === "master" && (
                            <> · 上次 {new Date(r.last_review_at).toLocaleDateString("zh-CN")}</>
                          )}
                        </div>
                      </div>
                      <Link
          ...
          {rows.length === PAGE_SIZE && (
            <p className="text-center text-xs text-muted-foreground">仅显示前 {PAGE_SIZE} 条 · 完成一些后再回来</p>
          )}
        </div>
      )}
    </main>
