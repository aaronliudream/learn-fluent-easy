...
{ key: "treasure", title: "宝藏关", emoji: "🎁", color: "from-amber-400 to-orange-500" }] as
const;
...
export default function PrimaryReadingPlay() {
  ...
  if (!a) return <main className="mx-auto max-w-3xl p-6"><p className="text-muted-foreground"><T>课文不存在</T></p><BackLink to="/primary" className="text-sm text-primary"><T>返回</T></BackLink></main>;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-6 pb-24">
      ...
      <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
          <span className="font-bold"><T>💡 亲子提示：</T></span>{a.parent_tip}
        </div>
      }
    </main>);
...
function WarmupStep({ warm, onDone }: {warm: Warm[];onDone: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>🌈 词卡热身</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>点一点每张卡片，听一听，再继续。</T></p>
...
function ListenStep({ sentences, onDone }: {sentences: Sentence[];onDone: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>👀 句子朗读</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>点 ▶︎ 自动播放整篇，跟着高亮的句子听。</T></p>
...
function ReadStep({ sentences, onDone }: {sentences: Sentence[];onDone: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>📣 跟我读</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>先听一遍，再大声跟读。读过的会变成 ✓。</T></p>
      <div className="mt-4 rounded-3xl bg-gradient-to-br from-emerald-50 to-teal-50 p-6 text-center">
        ...
        <button
          ...
          <Volume2 className="size-4" /> <T>听并标记已读</T>
        </button>
      </div>
      <div className="mt-3 flex items-center justify-between">
        <button onClick={() => setIdx((i) => Math.max(0, i - 1))} disabled={idx === 0}
        className="rounded-full border px-4 py-2 text-xs font-bold disabled:opacity-40"><T>← 上一句</T></button>
        <div className="text-xs text-muted-foreground">{idx + 1} / {sentences.length} <T>· 已读</T> {readSet.size}</div>
        <button onClick={() => setIdx((i) => Math.min(sentences.length - 1, i + 1))} disabled={idx === sentences.length - 1}
        className="rounded-full border px-4 py-2 text-xs font-bold disabled:opacity-40"><T>下一句 →</T></button>
      </div>
      <button
        onClick={onDone}
        disabled={!allRead}
...
function ThinkStep({
  ...
  const choose = (i: number) => {
    ...
    onAnswer?.(q, isC, i);
    if (isC) {setCorrect((c) => c + 1);onCorrect(idx);} else
    {onWrong(q);}
    setTimeout(() => {
      if (idx + 1 >= questions.length) onDone(correct + (i === q.answer ? 1 : 0));else
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>🤔 理解小问 (</T>{idx + 1}/{questions.length})</h3>
...
function BuildGame({ t, onDone, onCorrect }: {t: Extract<Treasure, {type: "build";}>;onDone: (ok: boolean) => void;onCorrect: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>🎁 宝藏关 · 拼一拼</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>按正确顺序点击单词，组成一句话。</T></p>
      <div className={cn(
        ...
        {built || <span className="text-muted-foreground"><T>点下方单词…</T></span>}
      </div>
      ...
      <div className="mt-4 flex gap-2">
        ...
        className="flex-1 rounded-2xl border-2 border-border px-4 py-2 text-xs font-bold"><T>清空</T></button>
        <button onClick={submit} disabled={picked.length !== pool.length || done !== null}
        className="flex-1 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-xs font-extrabold text-white disabled:opacity-50"><T>提交</T></button>
      </div>
    </div>);
...
function OrderGame({ t, onDone, onCorrect }: {t: Extract<Treasure, {type: "order";}>;onDone: (ok: boolean) => void;onCorrect: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>🎁 宝藏关 · 排一排</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>按故事顺序整理这些句子。</T></p>
      ...
      className="mt-4 w-full rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-3 text-sm font-extrabold text-white disabled:opacity-50"><T>提交</T></button>
    </div>);
...
function MatchGame({ t, onDone, onCorrect }: {t: Extract<Treasure, {type: "match";}>;onDone: (ok: boolean) => void;onCorrect: () => void;}) {
  ...
  return (
    <div>
      <h3 className="text-base font-extrabold"><T>🎁 宝藏关 · 配一配</T></h3>
      <p className="mt-1 text-xs text-muted-foreground"><T>点左边英文，再点右边中文，配成一对。</T></p>
  ...
  return (
    <div className="rounded-3xl border-2 border-border bg-card p-6 text-center shadow-tile">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        <T>准确率</T> {Math.round(accuracy * 100)}<T>% · 获得 +</T>{stars * 5} <T>星币</T>
      </p>
      ...
      className="mt-4 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-rose-500 to-amber-500 px-6 py-2.5 text-sm font-extrabold text-white shadow">
        <RotateCw className="size-4" /> <T>再读一遍</T>
      </button>
