// 美语课程 第四册 seed 生成器 —— 读 docs/american/book4/am4_l*.json → SQLAA/american_am4_seed_unitNN.sql
// 设计:JSON 为机器源(逐题 options[0]=正确项),消除 prose 解析歧义;本器只负责打散+落 SQL,内容不改。
// 打散口径与 am1/am2 完全一致:mulberry32 + assignAnswerPositions + placeQuota(答案位置配额 ≤⌊N/2⌋、无相邻重复)。
// 幂等:全部 ON CONFLICT。按 unit_no 分单元写文件,增课后重跑累积。第四册 6 课/单元(L1-6=U1…L43-48=U8),unit_no 由各课 JSON 自带。
// 用法: node scripts/american/gen-book4-seed.mjs
import { readFileSync, writeFileSync, mkdirSync, readdirSync, existsSync } from "node:fs";
import path from "node:path";

const ROOT = process.cwd();
const SRCDIR = path.join(ROOT, "docs/american/book4");

// ---------- 确定性 PRNG(mulberry32,与 am1/am2 生成器同实现,保证每次同一 SQL)----------
let _s = 0x9e3779b9;
function rand() {
...
// ---------- 载入所有 am4_l*.json ----------
const files = readdirSync(SRCDIR).filter((f) => /^am4_l\d+\.json$/.test(f)).sort();
if (!files.length) { console.error("docs/american/book4/ 下无 am4_l*.json"); process.exit(1); }
...
for (const U of units) {
  ...
  const hdr = [
    `-- 美语课程 第四册 seed · 单元 ${U}(生成器 gen-book4-seed.mjs 产出,勿手改;改 am4_l*.json 后重跑)`,
    `-- 幂等:全部 ON CONFLICT。依赖 SQLAA/american_add_book_no.sql 已跑(book_no 列)。`,
    `-- 本单元课:${ls.map((L) => L.id).join(", ")}`,
    "BEGIN;",
  ];
  const fp = path.join(ROOT, "SQLAA", `american_am4_seed_unit${p2(U)}.sql`);
...
console.log("\n========== 本次写文件判定(时间戳只在真变时才刷)==========");
if (written.length) for (const U of written) console.log(`  ✍️  需跑  american_am4_seed_unit${p2(U)}.sql(内容有变,已重写)`);
if (unchanged.length) for (const U of unchanged) console.log(`  ⏭️  无需重跑  american_am4_seed_unit${p2(U)}.sql(内容不变,未动文件)`);
