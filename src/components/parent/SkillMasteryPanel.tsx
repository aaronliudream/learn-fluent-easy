...
import {
  ...
  Loader2, Volume2, Target, ListChecks,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const rows: WordRow[] = vocabList.map(v => {
        ...
        else if (lvl >= 3) state = "mastered";
        else if (lvl >= 1) state = "learning";
        else if (correct + wrong > 0) state = wrong > correct ? "unmastered" : "learning";
    ...
    <section className="mb-4 rounded-3xl border-2 border-border bg-card p-4 shadow-tile md:p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-extrabold">📊 小学 · 各项技能掌握度</div>
            <div className="text-[11px] text-muted-foreground">看清孩子哪里掌握了 · 哪里还没掌握 · 一键继续学</div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl border-2 border-sky-200 bg-sky-50/50 p-3 dark:bg-sky-950/20">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 text-sm font-extrabold">
              <ListChecks className="size-4 text-sky-600" /> 词汇详情 · G{grade}
            </div>
            <div className="flex flex-wrap gap-1 text-[11px] font-bold">
              {([
                ...
                ["untouched", "⚪ 未学", vocabStat.untouched, "bg-slate-100 text-slate-700"],
          ...
          <div className="max-h-[420px] overflow-y-auto rounded-xl border border-border bg-card">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-secondary text-[10px] uppercase text-muted-foreground">
                <tr>
                  <th className="px-3 py-2 text-left">单词</th>
                  <th className="px-2 py-2 text-left">中文</th>
                  <th className="px-2 py-2 text-center">状态</th>
                  <th className="px-2 py-2 text-center">对/错</th>
                  <th className="px-2 py-2 text-right">最近</th>
                </tr>
              </thead>
              <tbody>
                {filteredWords.length === 0 ? (
                  <tr><td colSpan={5} className="py-8 text-center text-muted-foreground">该状态下没有单词 🎉</td></tr>
            ...
            {filteredWords.length > 100 && (
              <div className="border-t border-border/60 py-2 text-center text-[10px] text-muted-foreground">
                仅显示前 100 个 · 共 {filteredWords.length} 个
              </div>
            )}
          </div>
          ...
          <div className="mt-3 flex justify-end">
            ...
            >
              一键开始测验 <ChevronRight className="size-4" />
            </Link>
          </div>
        </div>
      )}
...
function SkillCard({
  ...
  stat?: SkillStat;
  customStats?: { label: string; value: string; color: string }[];
      ...
      {stat ? (
        <>
          ...
          <div className="grid grid-cols-4 gap-1 text-center text-[11px] font-bold">
            <div><div className="text-emerald-600">🟢 {stat.mastered}</div><div className="text-[9px] font-normal text-muted-foreground">掌握</div></div>
            <div><div className="text-amber-600">🟡 {stat.learning}</div><div className="text-[9px] font-normal text-muted-foreground">学习中</div></div>
            <div><div className="text-rose-600">🔴 {stat.unmastered}</div><div className="text-[9px] font-normal text-muted-foreground">未掌握</div></div>
            <div><div className="text-slate-600">⚪ {stat.untouched}</div><div className="text-[9px] font-normal text-muted-foreground">未学</div></div>
          </div>
        </>
      ) : (
