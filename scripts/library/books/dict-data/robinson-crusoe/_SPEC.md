...
{ "word": "habitation",
  ...
  "ex_cn": "他们在河边盖了个小住所。" }
```
- **gloss_cn 定义**:先看 `ctx` 判断书里用哪个义。**禁描述式**("一种…的动物/工具")——直接给对应中文词(perch→河鲈,不是"一种鱼");**末尾不带句号**;别机翻腔生硬。
- ⚠️ **多义/引申义(比 false-friend 更隐蔽)—— 本轮重点**:很多词有**同代多个引申义**,书里用的**不一定是最高频那个**。别只写最常见义 → 对着 `ctx` 看该词此处的**对象/用法**,**释义写得通用一点、能覆盖该词主要用法**,而不是只写最高频的一个。判据:**拿你的释义回读原句,逻辑通不通**。
  - 反例(Tom Sawyer 的 punctuate):原句 "needed breath to **punctuate the punches**"(对象是"一连串的捅",不是讲话)。只写"(讲话/文中)打断"→ 读者回读成"喘气来打断那些捅",不通。正确:`(用…)不时打断、间隔;为(一连串动作或话语)配上间隔的声响或强调`(通用、覆盖肢体+言语两种用法)。
