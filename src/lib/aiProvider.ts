...
// probe api.openai.com with a short timeout. If it succeeds → openai. If
// it fails or times out → qwen. The result is cached in localStorage so
// subsequent sessions skip the probe.

export type AIProvider = "openai" | "qwen";

const STORAGE_KEY = "fluentpath.aiProvider.v2";
const STORAGE_TS_KEY = "fluentpath.aiProvider.v2.ts";
 ...
 */
export async function probeOpenAIReachable(timeoutMs = 900): Promise<boolean> {
  try {
...
export async function resolveProvider(force = false): Promise<AIProvider> {
  ...
  const ok = await probeOpenAIReachable(isLikelyMainlandChinaBrowser() ? 1200 : 900);
