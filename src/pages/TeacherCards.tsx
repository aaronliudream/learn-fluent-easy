  ...
  if (!authed) {
    return (
      <main className="max-w-md mx-auto px-4 py-16 text-center space-y-4">
        ...
        <h1 className="text-2xl font-bold">老师 / 创作者面板</h1>
        <p className="text-muted-foreground">登录后查看你创建的知识卡数据</p>
        <Button onClick={() => navigate("/auth?redirect=/teacher/cards")}>登录</Button>
      </main>
    );
  }
    ...
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      {rows.length === 0 && (
        <Card className="p-8 text-center">
          <p className="text-muted-foreground mb-4">你还没有创建过知识卡</p>
          <Link to="/ask"><Button>提一个问题，生成第一张卡</Button></Link>
        </Card>
      )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 mt-2 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1">
                        <Eye className="w-3.5 h-3.5" />{r.view_count} 扫码
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <Users className="w-3.5 h-3.5" />{r.unique_users} 人答题
                      </span>
                      <span className="inline-flex items-center gap-1">
      ...
      <div className="text-center pt-4 border-t">
        <Link to="/ask">
          <Button variant="ghost"><Sparkles className="w-4 h-4 mr-2" />创建新卡片</Button>
        </Link>
      </div>
    </main>
  );
