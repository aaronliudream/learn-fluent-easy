    ...
    return (
      <section className="mb-4 rounded-3xl border-2 border-dashed border-primary/30 bg-primary/5 p-5 text-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-extrabold"><T>扫码答题学习</T></span>
        </div>
        <p className="text-muted-foreground mb-3">
          <T>还没有扫码答题记录。生成卡片分享给孩子，TA 答完题就会出现在这里。</T>
        </p>
        <Link to="/ask" className="inline-flex items-center gap-1 text-primary font-bold text-sm">
          <T>创建一张知识卡</T> <ChevronRight className="size-4" />
        </Link>
      </section>);
  ...
  return (
    <section className="mb-4 rounded-3xl border-2 border-primary/30 bg-gradient-to-br from-primary/10 via-primary/5 to-background p-5">
      <div className="flex items-center gap-2 mb-3">
        <Sparkles className="size-5 text-primary" />
        <h2 className="font-extrabold text-base"><T>扫码答题学习</T></h2>
        <span className="text-xs text-muted-foreground ml-auto"><T>最近 20 次</T></span>
      </div>
              ...
              <Link
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    ...
                    {r.correct_count}/{r.total_questions} <T>正确 · 第</T> {r.stage.replace("s", "")} <T>关</T>
