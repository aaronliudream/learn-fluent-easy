...
export default function JuniorListening() {
  ...
  const counts = useMemo(() => {
    ...
    for (const e of items) {
      ...
      if (s === "due") c.due += 1;
      else if (s === "learned") c.learned += 1;
      else if (s === "untouched") c.untouched += 1;
      else c.mastered += 1; // mastered + expert
    }
    return c;
  }, [items, aggMap]);
    ...
    <main className="mx-auto min-h-screen max-w-4xl px-5 py-8">
      ...
      <div className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">短对话 · 长对话 · 独白 · 填空 · 听写</p>
        </div>
      </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            {wrongCount > 0 && (
              <div className="flex items-center gap-3 rounded-2xl bg-white p-3 shadow-sm dark:bg-card">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">错题统计</div>
                  <div className="text-sm font-extrabold">累计答错 {wrongCount} 次</div>
                  <div className="text-[11px] text-muted-foreground">点 ⏰ 待复习筛选 · 集中订正</div>
                </div>
              </div>
            )}
          </div>
          ...
          <section key={sec.key} className="mt-7">
            <button onClick={() => setCollapsed(c => ({ ...c, [collapseKey]: !c[collapseKey] }))}
              ...
              <h2 className="text-base font-extrabold">
                ...
                {sec.emoji} {sec.label}
                <span className="ml-2 text-[11px] font-normal text-muted-foreground">{list.length} 段 · {sec.desc}</span>
              </h2>
            </button>
    ...
    <Link to={`/junior/listening/${e.id}`}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted-foreground">
          ...
          <span>· 难度 {"★".repeat(e.difficulty)}{"☆".repeat(Math.max(0, 3 - e.difficulty))}</span>
          {e.duration_sec ? <span>· {Math.round(e.duration_sec)}s</span> : null}
          {pct !== null && <span>· 上次 {pct}%</span>}
          {wrong > 0 && <span className="font-bold text-rose-600 dark:text-rose-400">· ❌ 累计 {wrong} 错</span>}
        </div>
      </div>
