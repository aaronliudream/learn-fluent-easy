...
export default function PrimaryLesson() {
  ...
  if (!lesson) return <main className="mx-auto max-w-3xl p-6"><p className="text-muted-foreground"><T>课程不存在</T></p><BackLink to="/primary" className="text-sm text-primary"><T>返回</T></BackLink></main>;
...
function QuizRunner({ questions, accent, onDone, label
...
}: {questions: {q: string;options: string[];answer: number;answers?: number[];}[];accent: string;onDone: (r: {correct: number;total: number;}) => void;label: string;onWrong?: (q: {q: string;options: string[];answer: number;}, picked: number) => void;}) {
  ...
  const pick = (k: number) => {
    ...
    setTimeout(() => {
      if (i + 1 >= questions.length) onDone({ correct: next.c, total: next.t });else
  ...
  return (
    <section className="space-y-4">
      ...
      <div className={`rounded-3xl border-2 bg-gradient-to-br ${accent} p-6 text-center text-white shadow-tile`}>
        <div className="text-[10px] font-bold uppercase tracking-wider opacity-80"><T>看题作答</T></div>
...
function OutputStep({ step, grade, onNext }: {step: Step;grade?: number;onNext: () => void;}) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span><T>🎤 跟我说 ·</T> {i + 1} / {items.length}</span>
      ...
      <div className="rounded-3xl border-2 border-pink-200 bg-gradient-to-br from-white to-pink-50 p-6 text-center shadow-tile">
        ...
        <p className="mt-3 text-[11px] text-muted-foreground"><T>先听一听，然后点下方麦克风跟读，AI 会打分 ✨</T></p>
      </div>
      ...
      <SpeakRecorder
        ...
        onPass={() => {
          const nd = [...done, i];
          setDone(nd);
          if (i + 1 >= items.length) onNext();else
          setI(i + 1);
        }}
        onSkip={() => {
          if (i + 1 >= items.length) onNext();else
...
function SpeakRecorder({ target, grade, scenario, onPass, onSkip
      ...
      <div className="space-y-3">
        <div className={`rounded-2xl bg-gradient-to-br ${tone} p-4 text-white shadow-tile`}>
          <div className="flex items-center justify-between">
            <div className="text-lg font-black">{s} <T>分</T></div>
          ...
          {result.transcript && <div className="mt-1 text-[11px] opacity-90"><T>你说的：</T>{result.transcript}</div>}
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => {setResult(null);setPhase("idle");}} className="rounded-2xl border-2 border-pink-200 bg-white py-3 text-sm font-extrabold text-pink-600"><T>再录一次</T></button>
          <button onClick={onPass} className="rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 py-3 text-sm font-extrabold text-white shadow-tile"><T>下一句 →</T></button>
        </div>
      </div>);
  ...
  if (phase === "scoring") {
    return (
      <div className="flex items-center justify-center gap-2 rounded-2xl border-2 border-pink-200 bg-pink-50 py-4 text-sm font-bold text-pink-600">
        <Loader2 className="size-4 animate-spin" /> <T>Spark 正在听你说…</T>
      </div>);
    ...
    return (
      <div className="space-y-2">
        ...
        className="w-full rounded-2xl bg-gradient-to-r from-rose-500 to-pink-500 py-4 text-base font-extrabold text-white shadow-tile animate-pulse">
          <span className="inline-flex items-center gap-2"><MicOff className="size-5" /> <T>录音中…点击结束（自动 6 秒）</T></span>
        </button>
      </div>);
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      <button onClick={onSkip} className="col-span-1 rounded-2xl border-2 border-muted bg-card py-3 text-xs font-bold text-muted-foreground"><T>跳过</T></button>
      ...
      className="col-span-2 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 py-4 text-base font-extrabold text-white shadow-tile">
        <span className="inline-flex items-center gap-2"><Mic className="size-5" /> <T>点我说一遍</T></span>
      </button>
    </div>);
  ...
  return (
    <section className="rounded-3xl border-2 border-amber-300 bg-gradient-to-br from-amber-50 via-rose-50 to-violet-50 p-8 text-center shadow-tile">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
          <T>测试答对</T> {testScore.c} / {testScore.t} <T>· 准确率</T> {Math.round(acc * 100)}%
        </p>
      }
      <div className="mt-4 inline-block rounded-full bg-gradient-to-r from-amber-400 to-rose-400 px-4 py-1.5 text-sm font-extrabold text-white">
        +{Math.round(acc * 100 * 0.5) + 20} <T>XP · Spark 亲密度 +5</T>
      </div>
