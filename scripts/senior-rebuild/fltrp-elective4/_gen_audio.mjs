// 外研社(fltrp)必修一 听力音频预生成(edge TTS nova/0.85),产出 audio_url UPDATE SQL。
// UPDATE 按 publisher+volume+unit+title(只回填 fltrp 本册,不动人教)。内容寻址 CF URL。
...
for (const u of Object.keys(META)) {
  const m = META[u];
  const lf = JSON.parse(readFileSync(`scripts/senior-rebuild/fltrp-${VOL}/${u}/${VOL}-${u}-listening.json`, 'utf8'));
...
let sql = `-- 外研社(fltrp)必修一 听力 audio_url 回填(${ok}/${total})。先跑各单元 load,再跑本文件。幂等。\nBEGIN;\n`;
...
writeFileSync(`SQLAA/fltrp-${VOL}-listening-audio-url.sql`, sql);
console.log(`\n合成 ${ok}/${total} -> SQLAA/fltrp-${VOL}-listening-audio-url.sql`);
