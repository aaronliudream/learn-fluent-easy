...
export default function PacingSettings() {
  ...
  async function saveProfile(next: {lessons_per_week?: number;study_days?: number[];}) {
    ...
    setSaving(false);
    if (error) toast.error("保存失败：" + error.message);else
    toast.success("已保存 ✓");
  }
  ...
  if (loading) {
    return (
      <section className="mb-4 rounded-3xl border-2 border-border bg-card p-4 text-sm text-muted-foreground">
        <Loader2 className="mr-2 inline size-4 animate-spin" /> <T>加载学习节奏…</T>
      </section>);
    ...
    <section className="mb-4 rounded-3xl border-2 border-border bg-card p-4 shadow-tile md:p-5">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-extrabold"><T>📅 学习节奏 & 个性化周计划</T></div>
            <div className="text-[11px] text-muted-foreground"><T>家长设置每周节奏，系统自动排课</T></div>
          </div>
        </div>
      ...
      <div className="mb-3 rounded-2xl border border-border bg-secondary/30 p-3">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="font-bold"><T>自定义：每周</T> <span className="text-violet-600">{perWeek}</span> <T>课</T></span>
          <span className="text-muted-foreground">{remaining} <T>课待学 · 预计</T> {weeksToFinish || "—"} <T>周完成</T></span>
        </div>
        <input
      ...
      {/* Study days */}
      <div className="mb-3">
        <div className="mb-1.5 text-xs font-bold"><T>学习日（点击选择）</T></div>
        ...
        <div className="mt-1 text-[10px] text-muted-foreground">
          <T>每周学习日 =</T> {days.length} <T>天 · 平均每天</T> {Math.ceil(perWeek / Math.max(1, days.length))} <T>课</T>
        </div>
      </div>
      ...
      <div className="rounded-2xl border-2 border-violet-200 bg-violet-50/50 p-3 dark:border-violet-800 dark:bg-violet-950/20">
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-sm font-extrabold">
            <Sparkles className="size-4 text-violet-600" /> <T>未来 4 周个性化课表 · G</T>{grade}
          </div>
              ...
              <Link
                ...
                className="flex items-center gap-2 rounded-lg border border-border/60 bg-secondary/40 p-2 text-xs transition hover:border-violet-400 hover:bg-violet-50 dark:hover:bg-violet-950/30">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[10px] text-muted-foreground">{it.lesson.unit.title_cn} · {it.lesson.estimated_minutes || 8} <T>分钟</T></div>
                      </div>
        ...
        <div className="mt-2 flex items-center gap-1 text-[10px] text-muted-foreground">
          <BookOpen className="size-3" /> <T>课表按「每周</T> {perWeek} <T>课 × 选定学习日」自动生成 · 完成一节自动进位</T>
        </div>
      </div>
    </section>);
