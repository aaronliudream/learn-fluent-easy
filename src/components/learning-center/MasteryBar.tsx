...
export function MasteryCounts({
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px] text-muted-foreground", className)}>
      <Dot color="bg-gps-master" /> <span>掌握 <strong className="text-foreground tabular-nums font-medium">{master}</strong></span>
      <Dot color="bg-gps-fluent" /> <span>熟练 <strong className="text-foreground tabular-nums font-medium">{fluent}</strong></span>
      <Dot color="bg-gps-weak" />   <span>薄弱 <strong className="text-foreground tabular-nums font-medium">{weak}</strong></span>
      <Dot color="bg-gps-none" />   <span>未学 <strong className="text-foreground tabular-nums font-medium">{none}</strong></span>
    </div>
  );
}
