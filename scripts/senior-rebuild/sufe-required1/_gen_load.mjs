// 生成上外(sufe)某单元灌库 SQL(junior_* 表,publisher='sufe')。用法: node _gen_load.mjs --unit u1
// ★所有 DELETE 都带 publisher='sufe' —— 绝不删人教(pep)行。所有 INSERT 显式写 publisher='sufe'。
// 唯一约束已含 publisher,与人教不撞。输出 -> SQLAA/sufe-required1-<unit>-load.sql。自带前后 COUNT 校验。
import { readFileSync, writeFileSync } from 'node:fs';
import { VOL, GRADE, META, CODE_PREFIX, PUBLISHER } from './_meta.mjs';
...
const rd = f => JSON.parse(readFileSync(`scripts/senior-rebuild/sufe-${VOL}/${U}/${VOL}-${U}-${f}.json`, 'utf8'));
...
let sql = `-- ============================================================\n-- 上外(sufe) 必修一 ${UNIT}(${m.title})灌库(junior_* 表,publisher='${PUB}' volume='${VOL}' unit='${UNIT}' grade=${GRADE})。幂等。\n`;
...
grammar.points.forEach((p, i) => {
  sql += `INSERT INTO public.junior_grammar_points (code, title, cefr, grade, summary, sort_order, volume, unit, category_id, examples, content_depth, publisher) VALUES (`
    + `'${esc(px(p.code))}', '${esc(circ[i] + p.point)}', 'A2', ${GRADE}, '${esc('上外必修一 ' + UNIT + ' — ' + p.overview)}', ${i + 1}, '${VOL}', '${UNIT}', (SELECT id FROM public.junior_grammar_categories WHERE code='senior' LIMIT 1), '[]'::jsonb, 0, '${PUB}');\n`;
});
...
listening.exercises.forEach(e => {
  ...
  sql += `INSERT INTO public.junior_listening_exercises (grade, title, topic, difficulty, transcript, translation_cn, speaker, questions, key_vocab, kind, volume, unit, publisher) VALUES (`
    + `${GRADE}, '${esc(e.title)}', '上外必修一 ${UNIT} · ${esc(e.type || '')}', ${e.kind === 'long' ? 3 : 2}, '${esc(tr)}', '${esc(e.translation_cn)}', '${esc(e.speaker || 'us_female')}', ${jb(qs)}, '[]'::jsonb, '${esc(e.kind)}', '${VOL}', '${UNIT}', '${PUB}');\n`;
});
...
writeFileSync(`scripts/sufe-required1-${U}-load.sql`, sql);
writeFileSync(`SQLAA/sufe-required1-${U}-load.sql`, sql);
const ins = (sql.match(/INSERT INTO/g) || []).length;
console.log(`上外 必修一 ${UNIT} 灌库 SQL -> SQLAA/sufe-required1-${U}-load.sql`);
