...
}: {itemId: string;module?: string;word?: string;correctAnswer?: string;}) {
    ...
    return (
      <div className="mt-3 rounded-2xl border-2 border-emerald-500/40 bg-emerald-500/10 p-4 text-sm">
        <div className="font-bold text-emerald-700 dark:text-emerald-300">
          <T>🌱 反思已记录，宠物正在消化…明天到账 +</T>{done.awarded}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          <T>研究表明：写下"我以为/真相/原因"比重做 5 次更牢固。</T>
        </div>
      </div>);
  ...
  return (
    <div className="mt-3 rounded-2xl border-2 border-fuchsia-500/30 bg-fuchsia-500/5 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-fuchsia-700 dark:text-fuchsia-300">
        <Sparkles className="size-4" /> <T>反思一下，领 2 颗种子 🌱</T>
      </div>
      <div className="space-y-2 text-sm">
        <label className="block">
          <span className="text-xs text-muted-foreground"><T>我以为答案是…</T></span>
          <input
            value={iThought}
        ...
        <label className="block">
          <span className="text-xs text-muted-foreground"><T>为什么会错？（可选）</T></span>
          <input
            value={why}
        ...
        <button
          ...
          {submitting ? <Loader2 className="size-3.5 animate-spin" /> : "🌱"} <T>领取反思能量</T>
        </button>
        <div className="text-[11px] text-muted-foreground">
          <T>每道题每天只能反思一次。明天 24h 后到账。</T>
        </div>
      </div>
    </div>);
